...
**Status:** Step 1 — Config centralization in progress
...
- Hosted reference (rerun 2025-12-26T20:34Z):  
  ...
  make
  ```
  Last lines: `✓ Build complete: build/amd64/standard/starforth ...`
- Kernel parity harness (rerun 2025-12-26T20:36Z):  
  ```bash
  STARFORTH_ENABLE_VM=1 PARITY_MODE=1 make -f Makefile.starkernel qemu
  ```
  Build succeeds; QEMU log still halts at the parity page fault (latest serial dump captured under “Kernel Log — 2025-12-26T20:37Z”).

### Progress Update — 2025-12-26 (Codex)
- ✅ Step 0 complete: both hosted + kernel builds re-run, log captured, no regressions relative to prior PF halt.
- 🧭 Step 1 plan locked (see “Refactor Plan — 2025-12-26” below); execution now focused on centralizing feature toggles without behavior changes.

### Step 1 — Config Centralization (2025-12-26T20:46Z)
...
### Step 2 — HAL Boundary (2025-12-26T21:17Z)
...
- `Makefile.starkernel` now pulls HAL sources via the existing `$(wildcard $(KERNEL_SRC)/hal/*.c)` sweep, so there’s a single definition point for HAL wiring regardless of LOADER vs kernel objects.
...
### Step 3 — VM Glue Relocation (2025-12-26T21:31Z)
...
### Step 4 — Header Isolation (2025-12-26T21:38Z)
...
### Step 5 — Build Options Map (2025-12-26T21:45Z)
- Audited every VM-facing flag; `include/starforth_config.h` now stands as the only default definition point for VM feature toggles while Makefiles merely pass overrides (`VM_FEATURE_FLAG_VARS` gates them so no duplicate defaults exist). Platform/test knobs live exclusively in the Makefiles (`PARITY_MODE`, `STARFORTH_ENABLE_VM`, `STARFORTH_ENABLE_TESTS`, `MONOLITHIC`).
- Documented ownership and effect of each build option in the new “Build Options Map” section below so reviewers know exactly where to set overrides and what each flag controls. Kernel-specific overrides (e.g., forcing `HEARTBEAT_THREAD_ENABLED=0`) are recorded there as well.
- Reconfirmed hosted + kernel builds (same commands) to ensure config cleanup didn’t change behavior; kernel still halts at the parity #PF while hosted build completes normally.

### Step 6 — HAL XT Validation (2025-12-26T21:53Z)
- Added a host-service hook (`VMHostServices.is_executable_ptr`) so the VM core can validate execution tokens without touching kernel headers. Hosted builds stub it to “always true”; kernel wires it to the new HAL query below.
- `sk_hal_is_executable_ptr()` now walks the live VMM page tables and enforces canonical, mapped, NX-clear pointers, logging the rejection reason plus kernel text range when an XT fails validation.
- `vm_create_word()` refuses to register any word whose function pointer fails the HAL check, logging the word name + XT address; this guards both builtin registrations and runtime definitions.
- Rebuilt hosted + kernel targets, reran the parity harness (`STARFORTH_ENABLE_VM=1 PARITY_MODE=1 timeout 60 make -f Makefile.starkernel qemu`), and captured the full serial log with timestamp below. Behavior is unchanged (same #PF), but XT validation now enforces the HAL boundary before parity runs.

### Refactor Plan — 2025-12-26
| Step | What moves | Why |
|------|------------|-----|
...
| 5 | Finalize build-option taxonomy + “Build Options Map” table | Document ownership/definition for every flag, prune duplicates, align defaults |
...
### Progress Update — 2025-12-26 (Claude)
...
- VM bootstrap/parity lives entirely under `src/starkernel/vm/` with the shared kernel HAL (`src/starkernel/hal/hal.c`) supplying memory/time/console hooks, the required `TICKS`, `HB`, and `YIELD` words, and a deterministic “`1 2 +`” smoke script before parity collection. The latest run still trips a #PF while `register_arithmetic_words` executes (`RIP=CR2=0x000a0000`), so the next blocker is debugging that low-page execution attempt before we can emit the updated parity packet.
...
| Flag | Default / Definition | Override Owner | Effect / Notes |
|------|---------------------|----------------|----------------|
| `PARITY_MODE` | Makefile / Makefile.starkernel (default 0) | Parity harness | Enables deterministic time + parity logging when set to 1. |
| `STARFORTH_ENABLE_VM` | `Makefile.starkernel` (opt-in) | Kernel integration | Pulls the hosted VM core + glue into the kernel image. |
| `STARFORTH_ENABLE_TESTS` | `Makefile.starkernel` (default off) | POST owner | Adds POST/test modules during parity POST runs. |
| `MONOLITHIC` | `Makefile.starkernel` (default 1) | Kernel build system | Controls whether loader+kernel are linked together. |
| `STRICT_PTR` | `include/starforth_config.h` | `make STRICT_PTR=0/1` | Selects pointer model for dictionary addresses. |
| `ENABLE_HOTWORDS_CACHE` | config header | `make ENABLE_HOTWORDS_CACHE=<0|1>` | Toggles physics hot-words cache instrumentation. |
| `ENABLE_PIPELINING` | config header | `make ENABLE_PIPELINING=<0|1>` | Enables speculative execution + transition metrics. |
| `ROLLING_WINDOW_SIZE` | config header | `make ROLLING_WINDOW_SIZE=<n>` | Initial history window size (Knob #7). |
| `TRANSITION_WINDOW_SIZE` | config header | `make TRANSITION_WINDOW_SIZE=<n>` | Word-transition context depth (Knob #6). |
| `ADAPTIVE_SHRINK_RATE` | config header | `make ADAPTIVE_SHRINK_RATE=<n>` | Percentage retained when shrinking history window. |
| `ADAPTIVE_MIN_WINDOW_SIZE` | config header | `make ADAPTIVE_MIN_WINDOW_SIZE=<n>` | Lower bound for adaptive shrinking. |
| `ADAPTIVE_CHECK_FREQUENCY` | config header | `make ADAPTIVE_CHECK_FREQUENCY=<n>` | Execution interval between diversity checks. |
| `ADAPTIVE_GROWTH_THRESHOLD` | config header | `make ADAPTIVE_GROWTH_THRESHOLD=<n>` | Growth % that triggers shrink logic. |
| `INITIAL_DECAY_SLOPE_Q48` | config header | `make INITIAL_DECAY_SLOPE_Q48=<q48>` | Cold-start decay slope (Q48.16). |
| `DECAY_MIN_INTERVAL` | config header | `make DECAY_MIN_INTERVAL=<ns>` | Minimum elapsed time before decay applies. |
| `DECAY_RATE_PER_US_Q16` | config header | `make DECAY_RATE_PER_US_Q16=<q16>` | Heat decay rate per microsecond (Q16). |
| `HEARTBEAT_THREAD_ENABLED` | config header default 1 (kernel Makefile forces 0) | Hosted builds may override; kernel fixed at 0 | Enables background heartbeat worker on hosted builds; kernel remains single-threaded to honor HAL boundary. |
| `HEARTBEAT_TICK_NS` | config header | `make HEARTBEAT_TICK_NS=<ns>` | Nominal heartbeat period. |
| `HEARTBEAT_INFERENCE_FREQUENCY` | config header | `make HEARTBEAT_INFERENCE_FREQUENCY=<ticks>` | Cadence for unified inference engine. |
| `HEARTBEAT_CHECK_FREQUENCY` | config header | `make HEARTBEAT_CHECK_FREQUENCY=<ticks>` | Inline vm_tick() cadence (when no worker). |
| `HEARTBEAT_WINDOW_TUNING_FREQUENCY` | config header | `make HEARTBEAT_WINDOW_TUNING_FREQUENCY=<ticks>` | Binary chop tuner interval. |
| `HEARTBEAT_SLOPE_VALIDATION_FREQUENCY` | config header | `make HEARTBEAT_SLOPE_VALIDATION_FREQUENCY=<ticks>` | Frequency of slope validation/logging. |

*Notes:* VM feature defaults exist only in `include/starforth_config.h`; the Makefiles pass `-D` overrides **only** when a caller supplies `make FLAG=value`. Platform/test flags (`PARITY_MODE`, `STARFORTH_ENABLE_*`, `MONOLITHIC`) live solely in the Makefiles. Kernel builds additionally force `HEARTBEAT_THREAD_ENABLED=0` so HAL guarantees remain intact.
...
## Kernel Log — 2025-12-26T21:53Z (Codex)
...
APIC Timer: apic_hz=1246414600, tick_hz=100, initial_count=12464146
...
RIP    : 0x000000000009ffff
CR2    : 0x00000000000a0000
Fault: Page Fault (#PF)
PF EC  : NP R SUP IFETCH 
Halting.
qemu-system-x86_64: terminating on signal 15 from pid 157020 (timeout)
make: *** [Makefile.starkernel:339: qemu] Terminated
```

## Kernel Log — 2025-12-26T21:38Z (Codex)
...
Heartbeat: init...
APIC Timer: calibrating...
APIC Timer: apic_hz=1247016200, tick_hz=100, initial_count=12470162
APIC Timer: configured (masked, ready to start)
Heartbeat: init done
Kernel initialization complete.
Boot successful!
...
PF EC  : NP R SUP IFETCH 
Halting.
qemu-system-x86_64: terminating on signal 15 from pid 154474 (timeout)
make: *** [Makefile.starkernel:339: qemu] Terminated
```

## Kernel Log — 2025-12-26T20:37Z (Codex)
...
### Note — Current Hand-off (2025-12-26T16:40Z)
- Interpreter gating + C-only vocabulary bring-up are in place; boot still crashes at `register_arithmetic_words`.
- Arena now has page-sized guard bands; the guard panic dumps the last installed word (name/XT/HERE/latest).
- Next session should: (1) use the new canary/logging hooks to catch the exact word that corrupts memory, (2) add per-slot invariants in `vm_create_word()`/threaded-store to fail early, and (3) build the minimal bootstrap harness so parity runs stop right after primitive registration for faster repro.
