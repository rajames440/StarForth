# Executive Summary: APIC Timer ISR Fix (amd64)
...
All three architectures boot to `ok>` with `PARITY:OK` and `Init: init.4th OK`.
amd64 additionally shows `Heartbeat: N ticks TIME-TRUST=0.999` repeating at 1 Hz.

```
grep "PARITY:OK"         logs/qemu-amd64-*.log
grep "Init: init.4th OK" logs/qemu-amd64-*.log
grep "ok>"               logs/qemu-amd64-*.log
