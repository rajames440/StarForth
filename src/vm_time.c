...
/*
 * vm_time.c - Heartbeat, timers, entropy/heat tracking, decay
 *
 * Time is a first-class liar — isolated here to keep the James Law
 * machinery from infecting everything else.
 *
 * Contains:
 *   - heartbeat logic
 *   - timers
 *   - entropy / heat tracking
 *   - decay / accumulation
 *   - any "tick" based logic
 */
...
void vm_tick_slope_validator(VM* vm)
{
    ...
    (void)stale_ratio; /* Suppress unused warning */
}
