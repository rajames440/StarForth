...
static void control_forth_if(VM *vm) {
    ...
    if (!cf_push(lit, CF_IF)) {
        vm->error = 1;
        log_message(LOG_ERROR, "IF: CF overflow");
        return;
    }
...
static void control_forth_else(VM *vm) {
    ...
    if (!cf_peek(&top)) {
        vm->error = 1;
        log_message(LOG_ERROR, "ELSE: CF underflow");
        return;
    }
    if (top.tag != CF_IF) {
        vm->error = 1;
    ...
    *(cell_t *) (vm->memory + top.addr) = off_bytes;
    ...
    if (!cf_push(new_lit, CF_ELSE)) {
        vm->error = 1;
        log_message(LOG_ERROR, "ELSE: CF overflow (new)");
        return;
    }
...
static void control_forth_then(VM *vm) {
    ...
    if (!cf_pop(&top)) {
        vm->error = 1;
        log_message(LOG_ERROR, "THEN: CF underflow");
        return;
    }
...
static void control_forth_begin(VM *vm) {
    ...
    if (!cf_push(vm->here, CF_BEGIN)) {
        vm->error = 1;
        log_message(LOG_ERROR, "BEGIN: CF overflow");
        return;
    }
...
static void control_forth_while(VM *vm) {
    ...
    if (!cf_peek(&top) || top.tag != CF_BEGIN) {
        vm->error = 1;
        log_message(LOG_ERROR, "WHILE: needs matching BEGIN");
        return;
    }
    ...
    if (!cf_push(lit, CF_WHILE)) {
        vm->error = 1;
        log_message(LOG_ERROR, "WHILE: CF overflow");
        return;
    }
...
static void control_forth_qdo(VM *vm) {
    ...
    if (!cf_push(vm->here, CF_DO)) {
        vm->error = 1;
        log_message(LOG_ERROR, "?DO: CF overflow (DO)");
        return;
    }
    if (!cf_push(fwd_lit, CF_WHILE)) {
        vm->error = 1;
        log_message(LOG_ERROR, "?DO: CF overflow (fwd)");
        return;
    }
    ...
    log_message(LOG_DEBUG, "?DO: fwd lit @ %zu, back mark @ %zu (leave_mark=%d)", fwd_lit, vm->here,
                leave_mark_stack[leave_mark_sp]);
}
...
static void control_forth_do(VM *vm) {
    ...
    if (!cf_push(vm->here, CF_DO)) {
        vm->error = 1;
        log_message(LOG_ERROR, "DO: CF overflow");
        return;
    }
...
static void control_forth_leave(VM *vm) {
    ...
    if (leave_sp + 1 >= CF_STACK_MAX) {
        vm->error = 1;
        log_message(LOG_ERROR, "LEAVE: too many leaves");
        return;
    }
...
static void control_forth_loop(VM *vm) {
    ...
    cf_item_t do_mark;
    if (!cf_pop(&do_mark) || do_mark.tag != CF_DO) {
        vm->error = 1;
    ...
    if (leave_mark_sp < 0) {
        vm->error = 1;
        log_message(LOG_ERROR, "LOOP: LEAVE mark stack underflow");
        return;
    }
...
static void control_forth_runtime_do(VM *vm) {
    if (vm->rsp < 0) {
        vm->error = 1;
        log_message(LOG_ERROR, "Runtime DO: RSP underflow");
        return;
    }
    ...
    if (vm->dsp < 1) {
        vm->error = 1;
        log_message(LOG_ERROR, "Runtime DO: DSP underflow");
        return;
    }
...
static void control_forth_runtime_loop(VM *vm) {
    if (vm->rsp < 0) {
        vm->error = 1;
        log_message(LOG_ERROR, "Runtime LOOP: RSP underflow");
        return;
    }
...
static void control_forth_runtime_plus_loop(VM *vm) {
    ...
    if (vm->dsp < 0) {
        vm->error = 1;
        log_message(LOG_ERROR, "Runtime +LOOP: DSP underflow");
        return;
    }
    cell_t n = vm_pop(vm);
