/*
   ...
   Path: src/word_source/control_words.c
   SPDX-License-Identifier: CC0-1.0

   Last modified - 2025-09-10
   Copyright (c) 2025 (rajames)
   ...
   - Compile-time: forward/backpatch via a small CF (control-flow) stack.
     Offsets are in BYTES, relative to the literal offset cell (classic threaded).
...
typedef enum {
    CF_BEGIN, /* mark: address of BEGIN target */
    CF_IF, /* mark: address of IF's 0BRANCH literal */
    CF_ELSE, /* mark: address of ELSE's BRANCH literal */
    CF_WHILE, /* mark: address of WHILE's 0BRANCH literal (paired with prior BEGIN) */
    CF_DO /* mark: address of loop body start (back target for LOOP/+LOOP) */
} cf_tag_t;

typedef struct {
    size_t addr; /* address (byte offset in vm->memory) used for patching/back edges */
    cf_tag_t tag;
} cf_item_t;
...
static size_t leave_addrs[CF_STACK_MAX];   /* addresses of BRANCH literals to patch */
static int leave_sp = -1;

/* One mark per DO nesting level: store the leave_sp value when entering DO/?DO.
   When LOOP/+LOOP occurs, patch all leaves above this mark and restore leave_sp. */
...
static inline size_t emit_cell(VM *vm, cell_t value) {
    if (vm->mode != MODE_COMPILE) {
        vm_push(vm, value); /* interpret: just push literal */
        return (size_t) -1;
    }
    vm_align(vm);
...
/* ===================== RUNTIME — RS-top holds per-step IP ===================== */
/* Layout after DO/?DO:  [..., LIMIT, INDEX, IP] with IP at the top of RS.       */
/* Offsets are BYTES, relative to the inline offset cell.                        */
...
static void control_forth_runtime_qdo(VM *vm) {
    ...
    if (vm->dsp < 1) {
        vm->error = 1;
        log_message(LOG_ERROR, "?DO: DSP underflow"); return;
    }

    /* pop index then limit (FORTH-79) */
    ...
    /* Enter loop: start at body (skip rel) */
    ip = ip + 1;

    /* Insert (limit,index) under IP: grow RS by 2, keep IP on top */
    if (vm->rsp + 2 >= STACK_SIZE) {
        vm->error = 1;
        log_message(LOG_ERROR, "?DO: RSTACK overflow");
        return;
    }
    vm->return_stack[vm->rsp + 2] = (cell_t) (uintptr_t) ip; /* move IP up */
    vm->return_stack[vm->rsp + 1] = index; /* under IP */
    vm->return_stack[vm->rsp + 0] = limit; /* deeper   */
    vm->rsp += 2;

    log_message(LOG_DEBUG, "?DO: enter (index=%ld limit=%ld)", (long) index, (long) limit);
}

/* (DO)  ( limit index -- ) — same insertion under IP; no inline offset */
static void control_forth_runtime_do(VM *vm) {
    if (vm->rsp < 0) {
        vm->error = 1;
        log_message(LOG_ERROR, "DO: RSP underflow");
    ...
    /* Current top-of-RS is already IP of loop body */
    if (vm->rsp + 2 >= STACK_SIZE) {
        vm->error = 1;
        log_message(LOG_ERROR, "DO: RSTACK overflow");
        return;
...
/* (LOOP)  ( -- ) — uses back-offset at IP; INDEX++, compare with LIMIT */
static void control_forth_runtime_loop(VM *vm) {
    if (vm->rsp < 2) {
        vm->error = 1;
...
static void control_forth_J(VM *vm) {
    if (vm->rsp < 4) { vm->error = 1; log_message(LOG_ERROR, "J: needs nested DO loops");
        return;
    }
    vm_push(vm, vm->return_stack[vm->rsp - 3]); /* outer INDEX */
}

/* EXIT — one-shot return from current colon definition */
static void control_forth_EXIT(VM *vm) {
    vm->exit_colon = 1;
...
/* IF ( flag -- ) compiles: 0BRANCH <fwd-placeholder> */
...
/* ELSE compiles: BRANCH <fwd-placeholder>; patches IF's 0BRANCH */
static void control_forth_else(VM *vm) {
    ...
    cf_item_t it;
    if (!cf_peek_item(&it) || it.tag != CF_IF) { vm->error = 1; log_message(LOG_ERROR, "ELSE: missing IF");
        return;
    }
    (void) cf_pop_item(&it);
...
static void control_forth_then(VM *vm) {
    ...
    if (!cf_pop_item(&it) || (it.tag != CF_IF && it.tag != CF_ELSE)) {
        vm->error = 1;
        log_message(LOG_ERROR, "THEN: unmatched"); return; }
    cell_t off = (cell_t)((size_t)vm->here - it.addr);
    *(cell_t *) (vm->memory + it.addr) = off;
...
static void control_forth_begin(VM *vm) {
    cf_epoch_sync(vm);
    if (vm->mode != MODE_COMPILE) { vm->error = 1;
...
static void control_forth_until(VM *vm) {
    ...
    cf_item_t begin;
    if (!cf_pop_item(&begin) || begin.tag != CF_BEGIN) { vm->error = 1; log_message(LOG_ERROR, "UNTIL: missing BEGIN");
        return;
    }
    vm_compile_call(vm, control_forth_0branch);
...
/* REPEAT compiles: BRANCH <back-to-BEGIN>, patches WHILE’s forward */
static void control_forth_repeat(VM *vm) {
    cf_epoch_sync(vm);
    if (vm->mode != MODE_COMPILE) {
        vm->error = 1;
        log_message(LOG_ERROR, "REPEAT: compile-only"); return; }
    cf_item_t w, b;
...
static void control_forth_leave(VM *vm) {
    ...
    /* verify inside DO */
    int seen_do = 0; for (int i = cf_sp; i >= 0; --i) {
        if (cf_stack[i].tag == CF_DO) {
            seen_do = 1;
            break;
        }
    }
    if (!seen_do || leave_mark_sp < 0) { vm->error = 1;
...
static void control_forth_plus_loop(VM *vm) {
    ...
    cf_item_t maybe_qdo; int have_qdo = 0;
    cf_item_t top;
    if (cf_peek_item(&top) && top.tag == CF_WHILE) { (void)cf_pop_item(&maybe_qdo); have_qdo = 1; }

    cf_item_t do_mark;
    if (!cf_pop_item(&do_mark) || do_mark.tag != CF_DO) { vm->error = 1; log_message(LOG_ERROR, "+LOOP: missing DO");
        return;
    }
...
void register_control_words(VM *vm) {
    ...
    log_message(LOG_INFO, "Registering FORTH-79 control flow words (two-stack runtime; no third stack)");
}
