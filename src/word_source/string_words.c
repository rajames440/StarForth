...
static void string_word_s_quote(VM *vm) {
    if (vm->dsp < 0) {
        vm->error = 1; log_message(LOG_ERROR, "S\": data stack underflow"); return;
    }
    ...
    if (!src || pos > end) {
        vm->error = 1; log_message(LOG_ERROR, "S\": input buffer not ready"); return;
    }
    ...
    if (pos >= end) {  /* no closing quote */
        vm->error = 1; log_message(LOG_ERROR, "S\": missing closing quote"); return;
    }

    size_t n = pos - start;
    if (n > 255) {
        vm->error = 1; log_message(LOG_ERROR, "S\": string too long (%zu)", n); return;
    }
    if (!vm_addr_ok(vm, dst, 1 + n)) {
        vm->error = 1; log_message(LOG_ERROR, "S\": dest out of bounds"); return;
    }
...
static void string_word_minus_trailing(VM *vm) {
    if (vm->dsp < 1) { vm->error = 1; log_message(LOG_ERROR, "-TRAILING: stack underflow"); return; }
    ...
    if (n <= 255 && vm_addr_ok(vm, a, 1) && vm_load_u8(vm, a) == (uint8_t)n) {
        if (!vm_addr_ok(vm, a + 1, n)) { vm->error = 1; log_message(LOG_ERROR, "-TRAILING: counted range OOB"); return; }
        s = a + 1;
    } else {
        if (!vm_addr_ok(vm, a, n)) { vm->error = 1; log_message(LOG_ERROR, "-TRAILING: range OOB"); return; }
    }
...
static void string_word_cmove(VM *vm) {
    ...
    if (u < 0) { vm->error = 1; log_message(LOG_ERROR, "CMOVE: negative count"); return; }
    if (u == 0) return;
...
static void string_word_compare(VM *vm) {
    if (vm->dsp < 3) { vm->error = 1; log_message(LOG_ERROR, "COMPARE: stack underflow"); return; }
    ...
    if (n1 <= 255 && vm_addr_ok(vm, a1, 1) && vm_load_u8(vm, a1) == (uint8_t)n1) {
        if (!vm_addr_ok(vm, a1 + 1, n1)) { vm->error = 1; log_message(LOG_ERROR, "COMPARE: s1 OOB"); return; }
        s1 = a1 + 1;
    } else {
        if (!vm_addr_ok(vm, a1, n1)) { vm->error = 1; log_message(LOG_ERROR, "COMPARE: s1 OOB"); return; }
    }

    vaddr_t s2 = a2;
    size_t  n2 = (size_t)u2;
    if (n2 <= 255 && vm_addr_ok(vm, a2, 1) && vm_load_u8(vm, a2) == (uint8_t)n2) {
        if (!vm_addr_ok(vm, a2 + 1, n2)) { vm->error = 1; log_message(LOG_ERROR, "COMPARE: s2 OOB"); return; }
        s2 = a2 + 1;
    } else {
        if (!vm_addr_ok(vm, a2, n2)) { vm->error = 1; log_message(LOG_ERROR, "COMPARE: s2 OOB"); return; }
    }
    ...
    for (size_t i = 0; i < m; i++) {
        ...
        if (c1 != c2) { result = (c1 < c2) ? -1 : +1; break; }
    }
    if (result == 0) {
        if (n1 < n2) result = -1;
...
static void string_word_search(VM *vm) {
    if (vm->dsp < 3) { vm->error = 1; log_message(LOG_ERROR, "SEARCH: stack underflow"); return; }
    ...
    vaddr_t s1 = a1; size_t n1 = (size_t)u1;
    if (n1 <= 255 && vm_addr_ok(vm, a1, 1) && vm_load_u8(vm, a1) == (uint8_t)n1) {
        if (!vm_addr_ok(vm, a1 + 1, n1)) { vm->error = 1; log_message(LOG_ERROR, "SEARCH: s1 OOB"); return; }
        s1 = a1 + 1;
    } else {
        if (!vm_addr_ok(vm, a1, n1)) { vm->error = 1; log_message(LOG_ERROR, "SEARCH: s1 OOB"); return; }
    }

    vaddr_t s2 = a2; size_t n2 = (size_t)u2;
    if (n2 <= 255 && vm_addr_ok(vm, a2, 1) && vm_load_u8(vm, a2) == (uint8_t)n2) {
        if (!vm_addr_ok(vm, a2 + 1, n2)) { vm->error = 1; log_message(LOG_ERROR, "SEARCH: s2 OOB"); return; }
        s2 = a2 + 1;
    } else {
        if (!vm_addr_ok(vm, a2, n2)) { vm->error = 1; log_message(LOG_ERROR, "SEARCH: s2 OOB"); return; }
    }
...
static void string_word_scan(VM *vm) {
    if (vm->dsp < 2) { vm->error = 1; log_message(LOG_ERROR, "SCAN: stack underflow"); return; }
    ...
    if (n <= 255 && vm_addr_ok(vm, a, 1) && vm_load_u8(vm, a) == (uint8_t)n) {
        if (!vm_addr_ok(vm, a + 1, n)) { vm->error = 1; log_message(LOG_ERROR, "SCAN: counted range OOB"); return; }
        s = a + 1;
    } else {
        if (!vm_addr_ok(vm, a, n)) { vm->error = 1; log_message(LOG_ERROR, "SCAN: range OOB"); return; }
    }
...
static void string_word_skip(VM *vm) {
    if (vm->dsp < 2) { vm->error = 1; log_message(LOG_ERROR, "SKIP: stack underflow"); return; }
    ...
    /* auto-detect counted string form (first byte equals u) */
    if (n <= 255 && vm_addr_ok(vm, a, 1) && vm_load_u8(vm, a) == (uint8_t)n) {
        if (!vm_addr_ok(vm, a + 1, n)) { vm->error = 1; log_message(LOG_ERROR, "SKIP: counted range OOB"); return; }
        s = a + 1;
    } else {
        if (!vm_addr_ok(vm, a, n)) { vm->error = 1; log_message(LOG_ERROR, "SKIP: range OOB"); return; }
    }
...
static void string_word_blank(VM *vm) {
    if (vm->dsp < 1) { vm->error = 1; log_message(LOG_ERROR, "BLANK: stack underflow"); return; }

    cell_t u    = vm_pop(vm);
    cell_t addr = vm_pop(vm);

    if (u < 0) { vm->error = 1; log_message(LOG_ERROR, "BLANK: negative length"); return; }
    if (u == 0) return;
    ...
    vaddr_t s = a;
    if (n <= 255 && vm_addr_ok(vm, a, 1) && vm_load_u8(vm, a) == (uint8_t)n) {
        if (!vm_addr_ok(vm, a + 1, n)) { vm->error = 1; log_message(LOG_ERROR, "BLANK: counted range OOB"); return; }
        s = a + 1;
    } else {
        if (!vm_addr_ok(vm, a, n)) { vm->error = 1; log_message(LOG_ERROR, "BLANK: range OOB"); return; }
    }
