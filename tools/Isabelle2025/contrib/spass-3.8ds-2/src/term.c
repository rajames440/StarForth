...
void term_StartBinding(void)   { term_BINDPHASE=TRUE; }
void term_StopBinding(void)    { term_BINDPHASE=FALSE; }
BOOL term_InBindingPhase(void) { return term_BINDPHASE; }
#else
 ...
 LIST term_ArgumentList(TERM T)
{
 ...
 void term_RplacArgumentListR(TERM T, LIST A)
{
  T->args = A;
}
...
#ifdef CHECK
  if ((symbol_IsFunction(term_TopSymbol(T)) || symbol_IsVariable(term_TopSymbol(T))) && !msorts_SortCheckTerm(T)) {
	  printf(" term_RplacArgumentList detected ill-sorted term: "); term_PrettyPrint(T); printf("\n");
	  misc_StartErrorReport();
	  misc_ErrorReport("\n In term_RplacArgumentList: Term ill-sorted.");
...
********************************************************/
{
  ...
  if (!symbol_IsSymbol(Symbol)) {
    misc_StartErrorReport();
    misc_ErrorReport("\n In term_Create: Illegal symbol.");
    misc_FinishErrorReport();
  }
  if (!term_IsTermList(List)) {
    misc_StartErrorReport();
    misc_ErrorReport("\n In term_Create: Illegal term list.");
    misc_FinishErrorReport();
  }
#endif
...
TERM term_CreateStandardVariable(SYMBOL Sort)
/**********************************************************
  RETURNS: Returns a term with a new variable of sort <Sort> as top symbol.
***********************************************************/
{
  return term_Create(symbol_MSortNextVariable(symbol_MSortLastVariable(Sort)), list_Nil());
}
  ...
  if (symbol_Equal(term_TopSymbol(Term), Symbol)) {
    ...
    for (List	= term_ArgumentList(Term);
	 list_Exist(List); List = list_Cdr(List))
      if (term_SubstituteVariable(Symbol, Repl, List->car))
	Result  = TRUE;
    return Result;
  }
}
...
SYMBOL term_GetSort(TERM Term)
/**************************************************************
  INPUT:   A term and a Sort
  RETURNS: <Sort> if term has sort <Sort> if term is a veriable or a function,
           NULL otherwise
***************************************************************/
{
	if (term_IsVariable(Term)) {
		SYMBOL sort= symbol_MSortVariableSort(term_TopSymbol(Term));
		if (sort >= 0) {
			printf("\nwrong sorted term: "); term_PrettyPrint(Term); printf("\n");
#ifdef CHECK
		    misc_StartErrorReport();
		    misc_ErrorReport("\n In term_GetSort: Unsorted Term.");
		    misc_FinishErrorReport();
#endif
		    return 0;
		}
		...
		return sort;
	} else if (symbol_IsFunction(term_TopSymbol(Term))) {
		SYMBOL sort= symbol_MFunctionSort(term_TopSymbol(Term));

		if (sort >= 0) {
			printf("\nwrong %d-sorted term: ",(int)sort); term_PrettyPrint(Term); printf("\n");
#ifdef CHECK
		    misc_StartErrorReport();
		    misc_ErrorReport("\n In term_GetSort: Unsorted Term.");
		    misc_FinishErrorReport();
...
BOOL term_CheckSort(TERM Term, SYMBOL Sort)
/**************************************************************
  INPUT:   A term and a Sort
  RETURNS: TRUE if term has sort <Sort> or is an atom, 
           FALSE otherwise
  CAUTION: <Term> is assumed to be no atom 
***************************************************************/
{
#ifdef CHECK
  if (!term_IsTerm(Term)) {
    misc_StartErrorReport();
    misc_ErrorReport("\n In term_SortCheck: Illegal input. Not a Term.");
    misc_FinishErrorReport();
  }
  if (term_IsAtom(Term)) {
    misc_StartErrorReport();
    misc_ErrorReport("\n In term_SortCheck: Illegal input. Is a Atom.");
    misc_FinishErrorReport();
  }  
#endif
  return fol_Top() == Sort || term_GetSort(Term) == Sort;
    /* if(term_IsVariable(Term)) {
      if(symbol_MSortVariableSort(term_TopSymbol(Term)) == Sort) {
	printf("\nTermsort %d, ref sort %d ", symbol_MSortVariableSort(term_TopSymbol(Term)), Sort);
	term_Print(Term);
	return TRUE;
      } else {
	return FALSE;
    ...
    } else if(symbol_MFunctionSort(term_TopSymbol(Term)) == Sort){
      printf("Termsort %d, ref sort %d ", symbol_MFunctionSort(term_TopSymbol(Term)), Sort);
      term_Print(Term);
      return TRUE;
    } else {
	return FALSE;
...
***************************************************************/
{
  ...
  if (!term_IsTerm(Term)) {
    misc_StartErrorReport();
    misc_ErrorReport("\n In term_Rename: Illegal input.");
    misc_FinishErrorReport();
  }
  if (term_InBindingPhase()) {
    misc_StartErrorReport();
    misc_ErrorReport("\n In term_Rename: Illegal context.");
    misc_FinishErrorReport();
  }
#endif
  ...
  do {
	...
	if (!term_VarIsMarked(Top, ActMark)) {
	  Sort = symbol_MSortVariableSort(Top);
	  term_CreateValueBinding(Top, ActMark, (POINTER)symbol_MSortGetNextActVar(Sort));
	}
...
***************************************************************/
{
  ...
  if (term_IsComplex(Term))
	...
	if (Depth != (NAT)term_BindingValue(Top)) {
	  Sort = symbol_MSortVariableSort(Top);
	  term_RplacTop(Term,symbol_MSortGetNextActVar(Sort));	
	}
...
***************************************************************/
{
  ...
  term_StartBinding();
  symbol_MSortSetActVarCounters(Var);
