...
/*     printf("\n");clause_PrintSpecial(list_Car(Scan)); */
  ...
  BOOL ok=TRUE;
//TODO sortcheck!  BOOL ok= msorts_SortCheckClauses(ReducedProof);
  ...
  if (flag_GetFlagIntValue(Flags, flag_TPTP) == flag_TPTPALL) {
    ...
    clause_ListPrintLabels(ReducedProof,ClauseToTermLabelList);
  ...
  if (ok == FALSE) {
	...
	misc_ErrorReport("\n In dp_PrintProof: Detected ill-sorted Proof.");
