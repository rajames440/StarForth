...
BOOL red_Tautology(CLAUSE Clause, FLAGSTORE Flags, PRECEDENCE Precedence)
/**********************************************************
 INPUT:   A non-empty clause, a flag store and a
	  precedence.
 RETURNS: The boolean value TRUE if 'Clause' is a
...
***************************************************************/
{
  ...
  j           = 0;
  Document    = flag_GetFlagIntValue(Flags, flag_DOCPROOF) || flag_GetFlagIntValue(Flags, flag_LIGHTDOCPROOF);
...
static BOOL  red_UNCSortsCompatible(LITERAL ActLit, LITERAL PLit)
/**************************************************************
  INPUT:   Two literals where the atoms are unifiable disregarding sorts.
  RETURNS: TRUE iff also the many-sorted conditions are met
***************************************************************/
{
  TERM Atom1, Atom2;

  Atom1 = clause_LiteralAtom(ActLit);
  Atom2 = clause_LiteralAtom(PLit);

  if (term_TopSymbol(Atom1) != fol_Equality())
    return TRUE;
  else {
    SYMBOL LeftSymb;
    LeftSymb = term_TopSymbol(term_FirstArgument(Atom1));
    if (symbol_IsVariable(LeftSymb)) {
      if (symbol_IsVariable(term_TopSymbol(term_FirstArgument(Atom2))))
	return symbol_Equal(symbol_MSortVariableSort(LeftSymb),
			    symbol_MSortVariableSort(term_TopSymbol(term_FirstArgument(Atom2))));
      else
	return symbol_Equal(symbol_MSortVariableSort(LeftSymb),
			    symbol_MFunctionSort(term_TopSymbol(term_FirstArgument(Atom2))));
    }
    else {
      if (symbol_IsVariable(term_TopSymbol(term_FirstArgument(Atom2))))
	return symbol_Equal(symbol_MFunctionSort(LeftSymb),
			    symbol_MSortVariableSort(term_TopSymbol(term_FirstArgument(Atom2))));
      else
	return symbol_Equal(symbol_MFunctionSort(LeftSymb),
			    symbol_MFunctionSort(term_TopSymbol(term_FirstArgument(Atom2))));
  ...
  misc_StartErrorReport();
  misc_ErrorReport("\n In red_UNCSortsCompatible:");
  misc_ErrorReport(" Unexpected Case.\n");
  misc_FinishErrorReport();
  ...
  if (clause_Length(Clause) == 1) {
	...
	while (!list_Empty(Scan)) {
	  ...
	  if (clause_LiteralsAreComplementary(ActLit,PLit) &&
	      ...
	      red_UNCSortsCompatible(ActLit, PLit)) {
	    st_CancelExistRetrieval();
	    Scan = list_Nil();
	  }
	  else {
...
***************************************************************/
{
//	printf("redunitclause: "); clause_Print(RedClause); printf("\n");
...
//		      printf("\tpot rew %d %d : ",clause_LiteralIsOrientedEquality(RewriteLit),TermS == term_FirstArgument(PartnerEq)); clause_Print(RewriteClause); printf("\n");
 ...
 ***************************************************************/
{
  ...
  Flags = prfs_Store(Search);
  Precedence = prfs_Precedence(Search);
  DocProof = flag_GetFlagIntValue(Flags, flag_DOCPROOF  || flag_GetFlagIntValue(Flags, flag_LIGHTDOCPROOF));
	...
	*Result = list_Cons(Copy, *Result);
	...
	for (LitScan = LitList;!list_Empty(LitScan);LitScan = list_Cdr(LitScan)) {
	  ...
	  if (list_PointerMember(Blocked,PClause)) {
	    if (!flag_GetFlagIntValue(Flags, flag_DOCPROOF)) //TODO  || flag_GetFlagIntValue(Flags, flag_LIGHTDOCPROOF)?
...
CLAUSE red_NegUnprotectedVarEquailtyHelp(CLAUSE clause, SYMBOL sym, TERM term, int index, PROOFSEARCH Search, FLAGSTORE Flags, PRECEDENCE Precedence) {
	CLAUSE tmp= clause_Copy(clause);
	clause_ReplaceVariable(tmp,sym,term);
	LIST NegLits= clause_CopyAntecedentExcept(tmp,index);
	LIST PosLits= clause_CopySuccedent(tmp);
	LIST ConstraintLits= clause_CopyConstraint(tmp);
	clause_Delete(tmp);
	CLAUSE new =clause_Create(ConstraintLits,NegLits,PosLits,Flags,Precedence);
	clause_SetDataFromFather(new,clause,index,Flags,Precedence);
	clause_SetDepth(new, clause_Depth(clause));
	clause_SetFromNegUnprotectedVarEQ(new);
	return new;
}

BOOL red_NegUnprotectedVarEquailty(CLAUSE clause,CLAUSE* new,  PROOFSEARCH Search, FLAGSTORE Flags, PRECEDENCE Precedence) {
	if (clause != NULL) {
		int end=clause_LastAntecedentLitIndex(clause);
		for (int i=clause_FirstAntecedentLitIndex(clause) ; i <= end; ++i) {
			LITERAL lit = clause_GetLiteral(clause, i);
			TERM eq= clause_LiteralAtom(lit);
			if (fol_IsEquality(eq)) {
				TERM lhs= term_FirstArgument(eq);
				TERM rhs= term_SecondArgument(eq);
				int literals= clause_NumOfAnteLits(clause)+clause_NumOfConsLits(clause)+clause_NumOfSuccLits(clause);
				literals= literals-1;
				if (literals == 0) {
					return FALSE;
				} else {
					SYMBOL l= term_TopSymbol(lhs);
					SYMBOL r= term_TopSymbol(rhs);
					if (symbol_IsVariable(l) && !term_ContainsVariable(rhs,l)) {
						*new= red_NegUnprotectedVarEquailtyHelp(clause,l,rhs,i,Search,Flags,Precedence);
						return TRUE;
					}
					if (symbol_IsVariable(r) && !term_ContainsVariable(lhs,r)) {
						*new= red_NegUnprotectedVarEquailtyHelp(clause,r,lhs,i,Search,Flags,Precedence);
						return TRUE;
					}
				}
			}
...
{ 
  ...
  while (oldClause != Clause) {
	  ...
	  /* Negative Unprotected Variabled Deletion */
	  while (flag_GetFlagIntValue(Flags,flag_RNUV) && red_NegUnprotectedVarEquailty(*Clause,&Copy, Search, Flags, Precedence)) {
	 ...
	 if (((flag_GetFlagIntValue(Flags, flag_RFREW) == flag_RFREWON) &&
	  ...
	  if (red_OnlyWorkedOffMode(Mode) && flag_GetFlagIntValue(Flags, flag_RSSI)) {
		red_SortSimplification(prfs_DynamicSortTheory(Search), *Clause,
				   prfs_LastBacktrackLevel(Search),
				   flag_GetFlagIntValue(Flags, flag_DOCPROOF)  || flag_GetFlagIntValue(Flags, flag_LIGHTDOCPROOF),
