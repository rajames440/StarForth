...
***************************************************************/
{
  ...
  printf(" Clause: %zd Card: %d Fire: %d Var: ", clause_Number(sort_LinkClause(Link)), sort_LinkCard(Link),
     sort_LinkFire(Link));
...
***************************************************************/
{
  ...
  Max    = symbol_MSortGetNextActVar(fol_Top());
...
***************************************************************/
{
  hash_helper = hm_Create(0, hm_PointerHash, hm_PointerEqual, FALSE);
...
*******************************************************/
{
  ...
  for (scan=subst;!subst_Empty(scan);scan=subst_Next(scan)) {
    ...
    Pair    = hm_Retrieve(hash_helper,subst_Cod(scan));    
...
*******************************************************/
{
  ...
  sharing_PushOnStack(Term);
    ...
    if (!(SortPair = (SOJU)hm_Retrieve(hash_helper,Term))) {
	...
	putchar(':');sort_PairPrint(SortPair);putchar('\n');*/
      ...
      hm_Insert(hash_helper,Term,SortPair);
    }
  }

  SortPair = sort_PairCopy(SortPair);
  hm_ClearWithElement(hash_helper,(void (*)(POINTER)) sort_DeleteSortPair);
