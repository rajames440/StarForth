...
BOOL unify_OccurCheckCom(SYMBOL Top, VARCONT Context, TERM Term)
/**********************************************************
...
BOOL unify_OccurCheck(VARCONT CTop, SYMBOL Top, VARCONT CTerm, TERM Term)
/**********************************************************
...
static BOOL unify_UnifyAux(BOOL Bound, VARCONT CtL, TERM TermL, VARCONT CtR, TERM TermR)
/**********************************************************
  INPUT:   Bool flag, two terms which may contain the same variable symbols.
           Equal variable symbols are interpreted different in
	         <TermL> and <TermR>, respectively.
           The flag specifies whether there has possibly been a binding before
           the function was called. (If not, the first occur-check test can be avoided.)
...
***********************************************************/
{
  ...
  for (;;) {
    ...
    if (stack_Empty(Bottom))
      ...
      TermR = list_Car(ArgsR);
      TermL = list_Car(ArgsL);
      CtR   = (VARCONT)stack_Top();
      CtL   = (VARCONT)stack_NthTop(1);
      if (list_Empty(list_Cdr(ArgsL)))
	stack_NPop(2);
      else {
	stack_Push(list_Cdr(ArgsL));
...
BOOL unify_Unify(VARCONT CtL, TERM TermL, VARCONT CtR, TERM TermR)
{ 
  return unify_UnifyAux(FALSE,CtL,TermL,CtR,TermR); 
}


BOOL unify_ContinueToUnify(VARCONT CtL, TERM TermL, VARCONT CtR, TERM TermR)
{ 
  return unify_UnifyAux(TRUE,CtL,TermL,CtR,TermR); 
}


BOOL unify_UnifyCom(VARCONT Context, TERM TermL, TERM TermR)
/**********************************************************
...
***********************************************************/
{
  ...
  for (;;) {
	...
	continue;
      ...
      if (term_IsVariable(TermL)) {
	if (term_CheckSort(TermR, term_GetSort(TermL))) {
	  if (term_IsVariable(TermR))
	...
	if (term_IsVariable(TermR) && term_CheckSort(TermL, term_GetSort(TermR))) {
...
BOOL unify_UnifyNoOC(VARCONT CtL, TERM TermL, VARCONT CtR, TERM TermR)
/**********************************************************
  INPUT:   Two terms which may contain the same variable symbols.
           Equal variable symbols are interpreted different in
	   <TermL> and <TermR>, respectively.
...
BOOL unify_UnifyAllOC(VARCONT IndexContext, VARCONT CtL, TERM TermL, VARCONT CtR, TERM TermR)
{
  while (term_IsVariable(TermL)) {
    SYMBOL TermTop;
...
BOOL unify_Match(VARCONT Context, TERM TermL, TERM TermR)
{
  if (term_IsVariable(TermL)) {
...
BOOL unify_MatchFlexible(VARCONT Context, TERM TermL, TERM TermR)
/**************************************************************
...
void unify_EstablishMatcher(VARCONT Context, SUBST Subst)
{
...
BOOL unify_MatchBindingsHelp(const VARCONT IndexContext, TERM TermL, VARCONT CtR, TERM TermR)
{
  while (term_IsVariable(TermR)) {
    SYMBOL TermTop;
...
BOOL unify_MatchBindings(const VARCONT IndexContext, TERM TermL, TERM TermR)
{
...
BOOL unify_MatchReverse(const VARCONT IndexContext, TERM TermL,	VARCONT CtR, 
			TERM TermR)
/*********************************************************
...
BOOL unify_Variation(const VARCONT Context, TERM TermL, TERM TermR)
{
	if (term_IsVariable(TermL)) {
		if (term_EqualTopSymbols(TermL, TermR))
...
BOOL unify_VariationVar(const VARCONT Context, TERM TermL, TERM TermR)
{
	if (term_IsVariable(TermL)) {
		if (term_EqualTopSymbols(TermL, TermR))
			/* TermL and TermR are in different contexts
	 ...
	 which is the term to be inserted. */
		...
		if (term_IsComplex(TermL)) {
			...
			for (ArgL = term_ArgumentList(TermL), ArgR = term_ArgumentList(TermR);
					!list_Empty(ArgL);
					ArgL = list_Cdr(ArgL), ArgR = list_Cdr(ArgR))
				if (!unify_VariationVar(Context, list_Car(ArgL), list_Car(ArgR))) {
					return FALSE;
				}
		}
		return TRUE;
...
TERM unify_ComGenLinear(const VARCONT IndexContext,
