...
void clause_SetParentClauses(CLAUSE Clause, LIST PClauses) {
    //	 printf("set partent clauses of "); clause_Print(Clause); printf("\n");

    Clause->parentCls = PClauses;
    for (LIST scan = Clause->parentCls; !list_Empty(scan); scan = list_Cdr(scan)) {
        nextclauseweights_addLabelToClauseFromClause(clause_Number(Clause), (intptr_t) list_Car(scan));
    }
}

void clause_AddParentClause(CLAUSE Clause, intptr_t PClause) {
    //	 printf("set parent of %d to %d ",clause_Number(Clause),PClause); clause_Print(Clause); printf("\n");
...
BOOL clause_IsFromFromNegUnprotectedVarEQ(CLAUSE Clause) {
    return Clause->origin == NEGUNPROTECTEDVAREQ_REDUCTION;
}
...
void clause_SetFromNegUnprotectedVarEQ(CLAUSE Clause) {
    Clause->origin = NEGUNPROTECTEDVAREQ_REDUCTION;
}
    ...
    if (clause_FirstLitIndex() != clause_FirstSuccedentLitIndex(Clause))
        ...
        for (i = clause_FirstLitIndex(); i < clause_Length(Clause); i++) {
            ...
            if (symbol_Equal(term_TopSymbol(Term), fol_Equality())) {
                if (symbol_IsVariable(term_TopSymbol(term_FirstArgument(Term)))) {
                    if (Sort == (SYMBOL) NULL)
                        Sort = symbol_MSortVariableSort(term_TopSymbol(term_FirstArgument(Term)));
                    else if (Sort != symbol_MSortVariableSort(term_TopSymbol(term_FirstArgument(Term))))
                        return FALSE;
                }
                if (symbol_IsVariable(term_TopSymbol(term_SecondArgument(Term)))) {
                    if (Sort == (SYMBOL) NULL)
                        Sort = symbol_MSortVariableSort(term_TopSymbol(term_SecondArgument(Term)));
                    else if (Sort != symbol_MSortVariableSort(term_TopSymbol(term_SecondArgument(Term))))
                        return FALSE;
                }
                if ((!symbol_IsVariable(term_TopSymbol(term_FirstArgument(Term))) &&
...
int clause_RankedWeight(CLAUSE clause) {
    return clause_Weight(clause) * nextclauseweights_getRank(clause) * (clause_Depth(clause) + 1);
}
...
LIST clause_InsertWeighedRank(CLAUSE Clause, LIST UsList, FLAGSTORE Flags,
                              PRECEDENCE Precedence)
/*********************************************************
  INPUT:   A clause, a list to insert the clause into,
           a flag store and a precedence.
...
**********************************************************/
{
    ...
    Weight = clause_RankedWeight(Clause);

    Scan = UsList;

    if (list_Empty(Scan) ||
        (clause_RankedWeight(list_Car(Scan)) > Weight)) {
        return list_Cons(Clause, Scan);
    } else {
        while (!list_Empty(list_Cdr(Scan)) &&
               (clause_RankedWeight(list_Car(list_Cdr(Scan))) <= Weight)) {
            Scan = list_Cdr(Scan);
        }
        list_Rplacd(Scan, list_Cons(Clause, list_Cdr(Scan)));
        return UsList;
...
LIST clause_ListSortWeighedRank(LIST Clauses)
/*********************************************************
  INPUT:   A list of clauses.
  RETURNS: The clause list sorted with respect to the weight
           of clauses, minimal weight first.
...
**********************************************************/
{
    ...
    for (Scan = Clauses; !list_Empty(Scan); Scan = list_Cdr(Scan)) {
        weight = clause_RankedWeight(list_Car(Scan));
        if (weight < clause_MAXWEIGHT)
            clause_SORT[weight] = list_Cons(list_Car(Scan), clause_SORT[weight]);
        else
            clause_SORT[clause_MAXWEIGHT] = list_Cons(list_Car(Scan), clause_SORT[clause_MAXWEIGHT]);
    ...
    Scan = list_NumberSort(clause_SORT[clause_MAXWEIGHT],
                           (NAT (*)(POINTER)) clause_RankedWeight);
    clause_SORT[clause_MAXWEIGHT] = list_Nil();
    for (weight = clause_MAXWEIGHT - 1; weight >= 0; weight--) {
        Scan = list_Nconc(clause_SORT[weight], Scan);
        clause_SORT[weight] = list_Nil();
...
**********************************************************/
{
    ...
    do {
        if (term_IsComplex(Term))
            ...
            MaxSym = ((MaxSym < (Help = term_TopSymbol(Term))) ? Help : MaxSym);
        while (!stack_Empty(Bottom) && list_Empty(stack_Top()))
            stack_Pop();
        if (!stack_Empty(Bottom)) {
            Term = (TERM) list_Car(stack_Top());
...
**********************************************************/
{
    ...
    do {
        if (term_IsComplex(Term))
            ...
            VarSym = ((VarSym < (Help = term_TopSymbol(Term))) ? Help : VarSym);
        while (!stack_Empty(Bottom) && list_Empty(stack_Top()))
            stack_Pop();
        if (!stack_Empty(Bottom)) {
            Term = (TERM) list_Car(stack_Top());
...
#ifdef CHECK
    if (!clause_IsUnorderedClause(Clause)) {
        /* Clause may be a byproduct of some hyper rule */
        misc_StartErrorReport();
        misc_ErrorReport("\n In clause_Delete:");
        misc_ErrorReport("\n Illegal input.");
        misc_FinishErrorReport();
...
***************************************************************/
{
    ...
    NewVar = term_CreateStandardVariable(fol_Top());
...
***************************************************************/
{
    ...
    if (clause_GetFlag(Clause, CONCLAUSE))
        printf("(d%d,C,r%d,rw%d) ", (int) clause_Depth(Clause), (int) nextclauseweights_getRank(Clause),
               (int) clause_RankedWeight(Clause));
    else
        printf("(d%d,A,r%d,rw%d) ", (int) clause_Depth(Clause), (int) nextclauseweights_getRank(Clause),
               (int) clause_RankedWeight(Clause));
...
void clause_ListPrintLabels(LIST ClauseList, HASHMAP ClauseToLabelMap)
/**************************************************************
  INPUT:   A list of clauses.
  RETURNS: Nothing.
  SUMMARY: Prints the clauses to stdout.
...
***************************************************************/
{
    for (; !(list_Empty(ClauseList)); ClauseList = list_Cdr(ClauseList)) {
        CLAUSE clause = list_Car(ClauseList);
        clause_PrintLabels(clause, ClauseToLabelMap);
        if (!list_Empty(ClauseList))
            putchar('\n');
    }
}
...
***************************************************************/
{
    ...
    switch (Result) {
        case NEGUNPROTECTEDVAREQ_REDUCTION: fputs("NuV", File);
            break;
        case CLAUSE_DELETION: fputs("App", File);
            break;
        case EMPTY_SORT: fputs("EmS", File);
            break;
        case SORT_RESOLUTION: fputs("SoR", File);
            break;
        case EQUALITY_RESOLUTION: fputs("EqR", File);
            break;
        case EQUALITY_FACTORING: fputs("EqF", File);
            break;
        case MERGING_PARAMODULATION: fputs("MPm", File);
            break;
        case SUPERPOSITION_RIGHT: fputs("SpR", File);
            break;
        case PARAMODULATION: fputs("SPm", File);
            break;
        case ORDERED_PARAMODULATION: fputs("OPm", File);
            break;
        case SUPERPOSITION_LEFT: fputs("SpL", File);
            break;
        case GENERAL_RESOLUTION: fputs("Res", File);
            break;
        case SIMPLE_HYPER: fputs("SHy", File);
            break;
        case ORDERED_HYPER: fputs("OHy", File);
            break;
        case UR_RESOLUTION: fputs("URR", File);
            break;
        case GENERAL_FACTORING: fputs("Fac", File);
            break;
        case SPLITTING: fputs("Spt", File);
            break;
        case INPUTCLAUSE: fputs("Inp", File);
            break;
        case REWRITING: fputs("Rew", File);
            break;
        case CONTEXTUAL_REWRITING: fputs("CRw", File);
            break;
        case CONDENSING: fputs("Con", File);
            break;
        case ASSIGNMENT_EQUATION_DELETION: fputs("AED", File);
            break;
        case OBVIOUS_REDUCTIONS: fputs("Obv", File);
            break;
        case SORT_SIMPLIFICATION: fputs("SSi", File);
            break;
        case MATCHING_REPLACEMENT_RESOLUTION: fputs("MRR", File);
            break;
        case UNIT_CONFLICT: fputs("UnC", File);
            break;
        case DEFAPPLICATION: fputs("Def", File);
            break;
        case TERMINATOR: fputs("Ter", File);
            break;
        case TEMPORARY: fputs("Temporary", File);
            break;
        case ORDERED_CHAINING: fputs("OCh", File);
            break;
        case NEGATIVE_CHAINING: fputs("NCh", File);
            break;
        case COMPOSITION_RESOLUTION: fputs("CRe", File);
            break;
        default:
            misc_StartErrorReport();
...
***************************************************************/
{
    if (clause_HasSolvedConstraint(Clause) &&
        ...
        (flag_GetFlagIntValue(Flags, flag_SELECT) != flag_SELECTIFSEVERALMAXIMAL || clause_NumberOfMaxLits(Clause) >
         1)) {
        NAT i, n;
        LITERAL Lit;
        ...
        if (flag_GetFlagIntValue(Flags, flag_SELECT) == flag_SELECTFROMLIST) {
            ...
            for (; i <= n; i++)
                if (symbol_HasProperty(term_TopSymbol(clause_LiteralAtom(clause_GetLiteral(Clause, i))), SELECTED)) {
                    Lit = clause_GetLiteral(Clause, i);
                    i = n + 1;
                } else if (clause_LiteralWeight(Lit)
