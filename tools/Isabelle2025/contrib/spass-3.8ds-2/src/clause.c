/*
  ...
  clause.c- FORTH-79 Standard and ANSI C99 ONLY
  Modified by - rajames
  Last modified - 2025-10-27T12:40:02.263-04
  ...
  /home/rajames/CLionProjects/StarForth/tools/Isabelle2025/contrib/spass-3.8ds-2/src/clause.c
 */
  ...
  if (clause_FirstLitIndex() != clause_FirstSuccedentLitIndex(Clause))
	...
	if (symbol_IsVariable(term_TopSymbol(term_FirstArgument(Term)))) {
	  if (Sort == (SYMBOL)NULL)
	    ...
	    if (Sort != symbol_MSortVariableSort(term_TopSymbol(term_FirstArgument(Term))))
	      return FALSE;
	}
	if (symbol_IsVariable(term_TopSymbol(term_SecondArgument(Term)))) {
	  if (Sort == (SYMBOL)NULL)
	    ...
	    if (Sort != symbol_MSortVariableSort(term_TopSymbol(term_SecondArgument(Term))))
	      return FALSE;
	}
	if ((!symbol_IsVariable(term_TopSymbol(term_FirstArgument(Term))) &&
...
***************************************************************/
{
  ...
  if (clause_GetFlag(Clause,CONCLAUSE))
	  printf("(d%d,C,r%d,rw%d) ",(int)clause_Depth(Clause),(int)nextclauseweights_getRank(Clause),(int)clause_RankedWeight(Clause));
  else
	  printf("(d%d,A,r%d,rw%d) ",(int)clause_Depth(Clause),(int)nextclauseweights_getRank(Clause),(int)clause_RankedWeight(Clause));
...
***************************************************************/
{
  ...
  switch(Result) {
  case NEGUNPROTECTEDVAREQ_REDUCTION:   fputs("NuV", File); break;
