...
BOOL fol_IsJunctor(SYMBOL S)
{
  return fol_IsQuantifier(S) || symbol_Equal(S, fol_AND) || 
    ...
    symbol_Equal(S, fol_XOR) || symbol_Equal(S, fol_NOR) ||
    symbol_Equal(S, fol_NAND) || symbol_Equal(S, fol_SUBSORT) ||
    symbol_Equal(S, fol_HASSORT) || symbol_Equal(S, fol_DIST) ||
    symbol_Equal(S, fol_DATATYPE) || symbol_Equal(S, fol_ANNOTATION) ||
    symbol_Equal(S, fol_CONST) || symbol_Equal(S, fol_LR)
    ;
}
...
{ 
  if (All) {
    ...
    fol_XOR      = symbol_CreateJunctor("xor", 2, symbol_STATLEX, Precedence);
    fol_NOR      = symbol_CreateJunctor("nor", 2, symbol_STATLEX, Precedence);
    fol_NAND     = symbol_CreateJunctor("nand", 2, symbol_STATLEX, Precedence);

    fol_SUBSORT  = symbol_CreateJunctor("subsort", 2, symbol_STATLEX, Precedence);
    fol_HASSORT  = symbol_CreateJunctor("hassort", 2, symbol_STATLEX, Precedence);
    fol_DIST     = symbol_CreateJunctor("distinct", 1, symbol_STATLEX, Precedence);
    fol_DATATYPE = symbol_CreateJunctor("datatype", 1, symbol_STATLEX, Precedence);
    
    fol_ANNOTATION = symbol_CreateJunctor("annotation", 2, symbol_STATLEX, Precedence);

    fol_CONST    = symbol_CreateJunctor("const", 2, symbol_STATLEX, Precedence);

    fol_PLUS     = symbol_CreateFunction("plus", 2, symbol_STATLEX, Precedence);
    fol_MINUS    = symbol_CreateFunction("minus", 2, symbol_STATLEX, Precedence);
    fol_MULT     = symbol_CreateFunction("mult", 2, symbol_STATLEX, Precedence);
    fol_FRACT    = symbol_CreateFunction("fract", 2, symbol_STATLEX, Precedence);
	...
	fol_NONEQUALITY = symbol_CreatePredicate("nequal", 2, symbol_STATLEX, Precedence);
    ...
    fol_LE       = symbol_CreatePredicate("le", 2, symbol_STATLEX, Precedence);
    fol_LS       = symbol_CreatePredicate("ls", 2, symbol_STATLEX, Precedence);
    fol_GE       = symbol_CreatePredicate("ge", 2, symbol_STATLEX, Precedence);
    fol_GS       = symbol_CreatePredicate("gs", 2, symbol_STATLEX, Precedence);

    fol_NATURAL  = symbol_CreatePredicate("Natural", 1, symbol_STATLEX, Precedence);
    fol_INTEGER  = symbol_CreatePredicate("Integer", 1, symbol_STATLEX, Precedence);
    fol_RATIONAL = symbol_CreatePredicate("Rational", 1, symbol_STATLEX, Precedence);
    fol_REAL     = symbol_CreatePredicate("Real", 1, symbol_STATLEX, Precedence);
    fol_TOP      = symbol_CreatePredicate("Top", 1, symbol_STATLEX, Precedence);

    fol_LR       = symbol_CreateJunctor("lr", 1, symbol_STATLEX, Precedence);

    symbol_MSortCreate(fol_NATURAL);
    symbol_MSortCreate(fol_INTEGER);
    symbol_MSortCreate(fol_RATIONAL);
    symbol_MSortCreate(fol_REAL);
    symbol_MSortCreate(fol_TOP);

    symbol_AddProperty(fol_EQUALITY, PREDEFINED);
    symbol_AddProperty(fol_NONEQUALITY, PREDEFINED);
    symbol_AddProperty(fol_TRUE, PREDEFINED);
    symbol_AddProperty(fol_FALSE, PREDEFINED);
	...
	list_Cons((POINTER)fol_AND, list_Cons((POINTER)fol_OR,
	  list_Cons((POINTER)fol_NOT,
	    list_Cons((POINTER)fol_IMPLIES, list_Cons((POINTER)fol_IMPLIED,
		 ...
		 list_Cons((POINTER)fol_SUBSORT, list_Cons((POINTER)fol_HASSORT,
		  list_Cons((POINTER)fol_DIST, list_Cons((POINTER)fol_DATATYPE,

	 list_Cons((POINTER)fol_ANNOTATION, list_Cons((POINTER)fol_NATURAL, 
          list_Cons((POINTER)fol_INTEGER, list_Cons((POINTER)fol_RATIONAL,
           list_Cons((POINTER)fol_REAL, list_Cons((POINTER)fol_TOP,

         list_Cons((POINTER)fol_LE, list_Cons((POINTER)fol_LS,
          list_Cons((POINTER)fol_GE, list_Cons((POINTER)fol_GS,
           list_Cons((POINTER)fol_PLUS, list_Cons((POINTER)fol_MINUS,
            list_Cons((POINTER)fol_MULT, list_Cons((POINTER)fol_FRACT,
             list_Cons((POINTER)fol_CONST,list_Cons((POINTER)fol_LR,
...
/**************************************************************
  INPUT:   A term.
  RETURNS: TRUE iff term is a literal.
***************************************************************/
{
...
***************************************************************/
{
  ...
  Result = list_DeleteElementIf(Result, (BOOL (*)(POINTER))symbol_IsPredefinedPred);
  return Result;
}
...
***************************************************************/
{
  ...
  if (term_IsComplex(Formula)) {
	...
	Sort    = symbol_MSortVariableSort(Var);
	OldVars = list_Nconc(OldVars,list_List((POINTER)term_BindingValue(Var)));
	term_CreateValueBinding(Var, term_OldMark(), (POINTER)symbol_MSortGetNextActVar(Sort));
      }
...
***************************************************************/
{
  symbol_MSortResetActVarCounters();
  term_NewMark();
...
***************************************************************/
{
  symbol_MSortSetActVarCounters(S); 
  term_NewMark();
  fol_NormalizeVarsIntern(Formula);
}
...
TERM fol_RemoveXorNorNand(TERM Formula)
/*********************************************************
  INPUT:   A formula.
  RETURNS: void.
  EFFECT:  All occurrences of "xor", "nor" and "nand" are 
            replecled by their respective equivalents.
  CAUTION: Destructive.
*******************************************************/
{
  if (!fol_IsLiteral(Formula)) {
    if (fol_IsQuantifier(term_TopSymbol(Formula)))
		term_RplacSecondArgument(Formula,fol_RemoveXorNorNand(term_SecondArgument(Formula)));
    else {
      LIST Scan;
	  
      if (symbol_Equal(term_TopSymbol(Formula),fol_Xor())) {
		  TERM arg1 = term_FirstArgument(Formula);
		  TERM arg2 = term_SecondArgument(Formula);

          TERM n_arg1 = term_Create(fol_Not(),list_List(term_Copy(arg1)));
          TERM n_arg2 = term_Create(fol_Not(),list_List(term_Copy(arg2)));

          TERM conj1 = term_Create(fol_And(),list_Cons(arg1,list_List(n_arg2)));
          TERM conj2 = term_Create(fol_And(),list_Cons(n_arg1,list_List(arg2)));

		  LIST args = term_ArgumentList(Formula);

		  list_Rplaca(args,conj1);
		  list_RplacSecond(args,conj2);
		  term_RplacTop(Formula,fol_Or());
		  
      } else if (symbol_Equal(term_TopSymbol(Formula),fol_Nor())) {		 
		  term_RplacTop(Formula,fol_Or());
		  
          Formula = term_Create(fol_Not(),list_List(Formula));
      } else if (symbol_Equal(term_TopSymbol(Formula),fol_Nand())) {
          term_RplacTop(Formula,fol_And());
          
          Formula = term_Create(fol_Not(),list_List(Formula));
      }
	        	 
      for (Scan=term_ArgumentList(Formula);!list_Empty(Scan);Scan=list_Cdr(Scan))
		list_Rplaca(Scan,fol_RemoveXorNorNand(list_Car(Scan)));		   
    }
  }	
...
LIST fol_GetPrecedence(PRECEDENCE Precedence)
/**************************************************************
  INPUT:   A precedence.
  RETURNS: the list of all signature symbols sorted in ascending order
...
***************************************************************/
{
  if (symbol_SignatureExists()) {
	...
	if  ((symbol_IsPredicate(Symbol) || symbol_IsFunction(Symbol)) &&
	     !symbol_IsPredefinedPred(Symbol))
...
***************************************************************/
{
  ...
  Symbols = fol_GetPrecedence(Precedence);
...
static void fol_ApplyContextToTermIntern(VARCONT Context, TERM Term)
/********************************************************************
...
static BOOL fol_CheckApplyContextToTerm(VARCONT Context, TERM Term)
/*************************************************************
...
BOOL fol_ApplyContextToTerm(VARCONT Context, TERM Term)
/***************************************************************
   INPUT:  A context (Context) and a term (Term).
...
*********************************************************************/
{
  ...
  do {
	...
	if (!symbol_IsJunctor(FormulaTop) && !symbol_IsJunctor(InstanceTop) &&
	    !symbol_IsPredefinedPred(FormulaTop) && !symbol_IsPredefinedPred(InstanceTop))
...
******************************************************************/
{
  ...
  do {
	...
	if (!symbol_IsJunctor(TermTop) && !symbol_IsJunctor(InstanceTop) &&
	    !symbol_IsPredefinedPred(TermTop) && !symbol_IsPredefinedPred(InstanceTop) &&
