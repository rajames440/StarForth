...
static void   cnf_OptimizedSkolemFormula(PROOFSEARCH, TERM, char*, BOOL, TERM,
					 LIST*, LIST*, BOOL, HASHMAP, int);
int    cnf_PredicateOccurrences(TERM, SYMBOL);
...
{  
  ...
  do {
    ...
    polterm1 = (intptr_t)vec_PopResult();
    if (term1 == subterm) {
      vec_SetMax(bottom);
      return polterm1;
    }else
...
********************************************************/
{
  ...
  if (symbol_Equal(term_TopSymbol(Def),fol_Equiv()) && (Polarity==1)) {
	...
	p = (intptr_t) list_PairSecond(pair);
...
int cnf_PredicateOccurrences(TERM Term, SYMBOL P)
/****************************************************
...
*************************************************************/
{
  ...
  if (maxvar_temp > maxvar)
    maxvar = maxvar_temp;
  symbol_MSortSetActVarCounters(maxvar);
  ...
  for (scan=bound; !list_Empty(scan); scan=list_Cdr(scan)) {
    ...
    if (term_ContainsSymbol(TargetTerm, term_TopSymbol(list_Car(scan))))
      term_ExchangeVariable(Formula, term_TopSymbol(list_Car(scan)),
			    symbol_MSortGetNextActVar(symbol_MSortVariableSort(term_TopSymbol(list_Car(scan)))));
  }
  list_Delete(bound);
...
***************************************************************/
{
  ...
  if (fol_IsQuantifier(term_TopSymbol(Term))) {
    if (symbol_Equal(Top,fol_All())) {
      ...
      for (Scan=fol_QuantifierVariables(Term);!list_Empty(Scan);Scan=list_Cdr(Scan)) {
	SkolemSymbol = symbol_CreateSkolemFunction(term_TopSymbol(list_Car(Scan)),Arity, Precedence);
...
static LIST cnf_GetUsedTerms(CLAUSE C, PROOFSEARCH Search,
			     HASHMAP InputClauseToTermLabellist) 
/**************************************************************
  INPUT:   
  RETURNS: 
***************************************************************/
{
  ...
  UsedTerms = list_Copy(hm_Retrieve(InputClauseToTermLabellist, C));
  ...
  for (Scan = clause_ParentClauses(C); !list_Empty(Scan); Scan = list_Cdr(Scan)) {
    CLAUSE P;
    int ClauseNumber;
    ClauseNumber = (intptr_t)list_Car(Scan);
...
***************************************************************/
{
  ...
  for (scan=Usables; !list_Empty(scan); scan = list_Cdr(scan)) {
    ...
    if (flag_GetFlagIntValue(Flags, flag_FPFCR) || flag_GetFlagIntValue(Flags, flag_DOCPROOF) || flag_GetFlagIntValue(Flags, flag_LIGHTDOCPROOF)) {
      hm_InsertListInsertUnique(InputClauseToTermLabellist, list_Car(scan), toplabel);
#ifdef CHECK_CNF
      fputs("\nUsable : ", stdout);
...
static TERM cnf_MakeSkolemFunction(SYMBOL Var, LIST varlist, PRECEDENCE Precedence)
/**************************************************************
  INPUT:   A varible to be replaced by a Skolem function, a list of variables and a precedence.
...
***************************************************************/
{
  ...
  skolem = symbol_CreateSkolemFunction(Var, list_Length(varlist), Precedence);
...
*******************************************************************/
{
  ...
  for (scan = VarList; scan != list_Nil(); scan = list_Cdr(scan)) {
    ...
    skolemfunction = cnf_MakeSkolemFunction(varsymb,FreeList, Precedence);
...
static void cnf_StrongSkolemization(PROOFSEARCH Search, TERM Topterm,
				    ...
				    LIST* UsedTerms, LIST* Symblist,
				    BOOL Result1,
				    HASHMAP InputClauseToTermLabellist, int Depth)
/******************************************************************
...
*******************************************************************/
{
  ...
  if (flag_GetFlagIntValue(Flags, flag_CNFSTRSKOLEM))
    symbol_MSortSetActVarCounters(term_MaxVar(Topterm));
  ...
  for (scan = exlist; !list_Empty(scan); scan = list_Cdr(scan)) {
    SYMBOL skolem;
    skolem = symbol_CreateSkolemFunction((SYMBOL)list_Car(scan),numberofallfreevariables, Precedence);
  ...
  for (i=0,scan = allfreevariables; i < numberofallfreevariables; i++, scan = list_Cdr(scan)) {
    w = term_CreateStandardVariable(symbol_MSortVariableSort(term_TopSymbol((TERM)list_Car(scan))));
...
***************************************************************/
{
  ...
  for (scan1=exlist; !list_Empty(scan1); scan1=list_Cdr(scan1)) {
    vec_Push(term);
    skolem = symbol_CreateSkolemFunction((SYMBOL)list_Car(scan1), n, Precedence);
...
**************************************************************/ 
{
  ...
  if (symbol_IsFunction(Top) && symbol_HasProperty(Top, SKOLEM)) {
	...
	SkolScan = list_Cdr(SkolScan);
      ...
      Var = symbol_MSortGetNextActVar(fol_Top());              /* This is probably to inprecise; real variable is needed */
...
**************************************************************/
{
  ...
  for (Scan = ArgList; Scan != list_Nil(); Scan = list_Cdr(Scan)) {
    ...
    if (list_Cdr(Vars) == list_Nil()) {
      SYMBOL New = symbol_MSortGetNextActVar(fol_Top());
...
**************************************************************/
{
  ...
  Vars = list_List((POINTER)symbol_MSortGetNextActVar(fol_Top()));
...
PROOFSEARCH cnf_Flotter(LIST AxiomList, LIST ConjectureList, LIST* AxClauses, 
			LIST* AllLabels, HASHMAP TermLabelToClauselist, 
			HASHMAP ClauseToTermLabellist, FLAGSTORE InputFlags,
...
/**************************************************************
  INPUT:   A list of axiom formulae,
	   ...
	   a hashmap in which for every term label the list of clauses 
	   derived from the term is stored (if DocProof is set),
	   a hashmap in which for every clause the list of labels 
...
***************************************************************/
{
  ...
  Result           = FALSE;
  if (flag_GetFlagIntValue(Flags, flag_FPFCR) || flag_GetFlagIntValue(Flags, flag_DOCPROOF) || flag_GetFlagIntValue(Flags,flag_LIGHTDOCPROOF))
    InputClauseToTermLabellist = hm_Create(0, hm_PointerHash, hm_PointerEqual, FALSE);
  else 
  ...
  for (Scan = ConjectureList; !list_Empty(Scan); Scan = list_Cdr(Scan)) {
    ...
    if (list_PairFirst(list_Car(Scan)) == NULL) {
      sprintf(buf, "conjecture%zu", Count);
      Label = string_StringCopy(buf);
      list_Rplaca((LIST) list_Car(Scan), Label);
      if ((flag_GetFlagIntValue(Flags, flag_DOCPROOF)  || flag_GetFlagIntValue(Flags, flag_FLOTTER) || flag_GetFlagIntValue(Flags, flag_LIGHTDOCPROOF)) &&
  ...
  if (flag_GetFlagIntValue(Flags, flag_CNFOPTSKOLEM) == flag_ON) {   
	...
	hm_InsertListInsertUnique(InputClauseToTermLabellist, list_Car(Scan2), list_PairFirst(list_Car(Scan)));
      }
    }
  ...
  *Symblist = list_Nil();
  for (Scan = AllFormulae; !list_Empty(Scan); Scan = list_Cdr(Scan)) {
    ...
    if (flag_GetFlagIntValue(Flags, flag_FPFCR) || flag_GetFlagIntValue(Flags, flag_DOCPROOF) || flag_GetFlagIntValue(Flags,flag_LIGHTDOCPROOF)) {
      hm_InsertListAppend(TermLabelToClauselist, list_PairFirst(Pair),list_Copy(Ax));
      UsedTerms = list_Cons(list_PairFirst(Pair), UsedTerms);
      UsedTerms = cnf_DeleteDuplicateLabelsFromList(UsedTerms);
      for (Scan2 = Ax; !list_Empty(Scan2); Scan2 = list_Cdr(Scan2)) {
	hm_InsertListAppend(ClauseToTermLabellist, list_Car(Scan2), list_Copy(UsedTerms));
	hm_InsertListAppend(InputClauseToTermLabellist, list_Car(Scan2), list_Copy(UsedTerms));
      } 
    }
  ...
  if (flag_GetFlagIntValue(Flags, flag_FPFCR) || flag_GetFlagIntValue(Flags, flag_DOCPROOF) || flag_GetFlagIntValue(Flags,flag_LIGHTDOCPROOF))
    hm_DeleteList(InputClauseToTermLabellist);
...
***************************************************************/
{
  ...
  flag_TransferFlag(Flags, SubProofFlags, flag_LIGHTDOCPROOF);
  ...
  BOOL ldp=FALSE;
  if (flag_GetFlagIntValue(SubProofFlags, flag_LIGHTDOCPROOF) )
    ldp = TRUE;

  //TODO LIGHTDOCPROOF
  flag_SetFlagIntValue(SubProofFlags, flag_DOCPROOF, flag_DOCPROOFOFF);
  flag_SetFlagIntValue(SubProofFlags, flag_LIGHTDOCPROOF, flag_LIGHTDOCPROOFOFF);
  ResultClauses = cnf_OptimizedSkolemization(cnf_SEARCHCOPY, term_Copy(Term),
					     NULL, &Dummy, Symblist, Result,
					     FALSE, NULL);

  if (ldp)
	flag_SetFlagIntValue(SubProofFlags, flag_LIGHTDOCPROOF, flag_LIGHTDOCPROOFON);
  if (Formulae2Clause)
...

{
  ...
  *LocallyTrue = FALSE;
  ...
  for (l1=vars; !list_Empty(l1); l1=list_Cdr(l1))
    term_ExchangeVariable(ToProveDef, term_TopSymbol(list_Car(l1)),  symbol_MSortGetNextActVar(symbol_MSortVariableSort((SYMBOL)list_Car(l1))));
  list_Delete(vars);
