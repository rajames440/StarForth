...
/**************************************************************
  INPUT:   A list and an element pointer and an equality test for two elements.
  RETURNS: The first element in the list that was found
           or NULL if none was.
***************************************************************/
{
  while (!list_Empty(List)) {
...
static  BOOL list_PointerLower (POINTER A, POINTER B)
{
  return (NAT) A < (NAT) B;
}
...
static  BOOL list_PointerNumberedLower(POINTER A, POINTER B) 
{
  return (BOOL) (NumberFunction(A) < NumberFunction(B));
}

static  BOOL list_PointerNumberedLowerOrEqual(POINTER A, POINTER B) 
{
  return (BOOL) (NumberFunction(A) <= NumberFunction(B));
}

static  BOOL list_PointerNumberedGreater(POINTER A, POINTER B) 
{
  return (BOOL) (NumberFunction(A) > NumberFunction(B));
}
...
{ 
  ...
  if (!list_Empty(List1)) {
    for (Scan=List2; !list_Empty(Scan) && !list_Empty(List1); Scan=list_Cdr(Scan))
...
***************************************************************/
{
  ...
  for (Scan = Multiset; !list_Empty(Scan); Scan = list_Cdr(Scan)) {
    ...
    if (Count != list_PairNull()) {
      Occurences = (intptr_t)list_PairSecond(Count);
...
***************************************************************/
{
  if ((intptr_t) list_PairSecond(LeftPair) < (intptr_t) list_PairSecond(RightPair)) {
    return -1;
  }
  else if ((intptr_t) list_PairSecond(LeftPair) > (intptr_t)list_PairSecond(RightPair)) {
    return 1;
  }
...
int list_CompareDistributions(LIST Left, LIST Right) 
/**************************************************************
