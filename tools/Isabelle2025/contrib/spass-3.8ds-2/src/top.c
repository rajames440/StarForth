...
CLAUSE top_SelectClauseDepth(LIST List, FLAGSTORE Flags)
/**************************************************************
 ...
 ***************************************************************/
{
	...
	if (flag_GetFlagIntValue(Flags, flag_PREFVAR) != flag_PREFVARDC)
 ...
 ***************************************************************/
{
	...
	for (Scan1=Result;!list_Empty(Scan1);Scan1=list_Cdr(Scan1))
		for (Scan2=list_Car(Scan1);!list_Empty(Scan2);Scan2=list_Cdr(Scan2))
			list_Rplaca(Scan2,clause_GetLiteral(Clause,(intptr_t)list_Car(Scan2)));
 ...
 ****************************************************************/
{
	...
	if (clause_HasSolvedConstraint(Clause) &&
			...
			(!clause_IsHornClause(Clause) ||
					(!ana_GetFeature(prfs_GetAnalyze(Search),ana_PUREPROPOSITIONAL) &&
 ...
 ***************************************************************/
{
	...
	if (MaxInstances > 0.0 &&
			(ana_GetFeature(prfs_GetAnalyze(Search),ana_PUREPROPOSITIONAL) ||
					MaxInstances >= flag_GetFlagIntValue(prfs_Store(Search), flag_SPLITMININST)))
...
static LIST top_FullReductionSelectGivenComputeDerivables(PROOFSEARCH Search,
		CLAUSE *SplitClause,
		int *Counter, int *MinInstances, HASHMAP ClauseToLabelMap)
/**************************************************************
	...
	if (GivenClause == NULL) {
		...
		if (GivenClause != NULL) {
			...
			*MinInstances = flag_GetFlagIntValue(Flags, flag_SPLITMININST);
			...
			if ( flag_GetFlagIntValue(prfs_Store(Search),flag_HEURISTIC) == flag_HEURISTICGOAL) {
				//			clock_StartCounter(clock_NEXTCLAUSE);
				GivenClause= nextclause_GoalOrientedGet(Search);
				//			clock_StopAddPassedTime(clock_NEXTCLAUSE);
				//			printf("\nnextclause spent time: "); clock_PrintTime(clock_NEXTCLAUSE); printf("\n");

		    } else if ( flag_GetFlagIntValue(prfs_Store(Search),flag_HEURISTIC) == flag_HEURISTICRANK) {
		    	GivenClause= nextclause_RankAidedGet(Search);
		    } else {
			...
			//			if (flag_GetFlagIntValue(Flags,flag_LIGHTDOCPROOF)) {
			//				nextclauseweights_printLabels(GivenClause);
			//				nextclauseweights_printRank(GivenClause);
			//				nextclauseweights_addClause(GivenClause);
			//				nextclauseweights_printRank(GivenClause);
			//			}
		}
	}
...
//	}
	...
	if (flag_GetFlagIntValue(Flags, flag_PGIVEN)) {
		fputs("\n\tGiven clause: ", stdout);
		clause_PrintLabels(GivenClause,ClauseToLabelMap);
		fflush(stdout);
	}
      ...
      clause_Print(GivenClause);printf("\n");*/
		...
		if (flag_GetFlagIntValue(Flags, flag_RTER) != flag_RTEROFF) {
			...
			if (TerminatorClause != NULL) {
				...
				prfs_InsertUsableClause(Search, GivenClause,TRUE);
			}
		}
		if (list_Empty(Derivables)) {
			/* No splitting was applied, no empty clause was found by terminator */
			/*clause_SetSpecialFlags(GivenClause,ana_GetFeature(prfs_GetAnalyze(Search),ana_SORTDECEQUATIONS));*/
...
static PROOFSEARCH top_ProofSearch(PROOFSEARCH Search, LIST ProblemClauses,
		...
		int *BoundApplied, BOOL NativeClauseInput, HASHMAP ClauseToLabelMap)
/**************************************************************
  ...
  EFFECTS: The <ProblemClauses> list is deleted by the function.
 ***************************************************************/
{
	nextclause_findConjectures(ProblemClauses);
	...
	clock_InitCounter(clock_NEXTCLAUSE);


	/* Important ! Recomputes Weight ! */
	ana_AnalyzeProblem(prfs_GetAnalyze(Search), prfs_Store(Search), ProblemClauses);
	if (flag_GetFlagIntValue(Flags, flag_AUTO)) {
		prfs_InstallFiniteMonadicPredicates(Search, ProblemClauses, ana_GetFinMonPredList(prfs_GetAnalyze(Search)));
		ana_AutoConfiguration(prfs_GetAnalyze(Search),ProblemClauses, Flags, Precedence);
		/* Nextclause flags have higher precedence */
		nextclause_setFlags(InputFlags,Flags);
	...
	if (flag_GetFlagIntValue(Flags,flag_LR)) {
		/* Rearrange automatically determined precedence according to problems's LR specification. */
		LIST ord= msorts_solveLR(NULL,nextclause_getAllowedSymbolCopy());
		symbol_RearrangePrecedence(Precedence, ord);
	}
	...
	if (flag_GetFlagIntValue(Flags, flag_LIGHTDOCPROOF)) {
		nextclauseweights_addInputClauses(ProblemClauses,ClauseToLabelMap);
	}
	if (flag_GetFlagIntValue(Flags, flag_PPROBLEM)) { /* we print the problem clauses before the transitivity axioms get potentially removed */
		puts("");
		puts("--------------------------SPASS-START-----------------------------");
		puts("Input Problem:");
		clause_ListPrintLabels(ProblemClauses,ClauseToLabelMap);
	}

	if (flag_GetFlagIntValue(Flags,flag_Transitivity))
		ProblemClauses = ana_AnalyzeTransitivity(prfs_GetAnalyze(Search),ProblemClauses);
	...
	if (NativeClauseInput) {
		...
		for (Scan = ProblemClauses; !list_Empty(Scan); Scan = list_Cdr(Scan)) {
			CLAUSE clause = list_Car(Scan);
			clause_PrecomputeOrderingAndReInit(list_Car(Scan), Flags, Precedence);
			nextclause_HACKFORCELT(Search,clause);
		}
	/* Must be called after ana_AnalyzeProblem and Reorientation */
	ana_AnalyzeSortStructure(prfs_GetAnalyze(Search),ProblemClauses, Flags, Precedence);

	if (flag_GetFlagIntValue(Flags, flag_AUTO)) {
		ana_ExploitSortAnalysis(prfs_GetAnalyze(Search),Flags);
		/* Nextclause flags have higher precedence */
		nextclause_setFlags(InputFlags,Flags);
		/* File and input flags have higher precedence */
		flag_TransferSetFlags(InputFlags, Flags);
	...
	*BoundApplied  = -1;
	...
	if (flag_GetFlagIntValue(Flags, flag_PPROBLEM)) {    /* the rest of the printing */
		ana_Print(prfs_GetAnalyze(Search), Flags, Precedence);
	}
	...
	if (ana_GetFeature(prfs_GetAnalyze(Search),ana_SORTRES) ||
			(ana_GetFeature(prfs_GetAnalyze(Search),ana_USORTRES) &&
		...
		while ((list_Empty(EmptyClauses) || !prfs_SplitStackEmpty(Search)) &&
			...
			if (!list_Empty(EmptyClauses)) {
				...
				if (prfs_SplitStackEmpty(Search)) {
					...
					/* for the proof.                                              */
					if (flag_GetFlagIntValue(Flags, flag_DOCPROOF)  || flag_GetFlagIntValue(Flags, flag_LIGHTDOCPROOF))
				...
				if (flag_GetFlagIntValue(Flags, flag_FULLRED))
					Derivables = top_FullReductionSelectGivenComputeDerivables(Search, &SplitClause, &Counter, &MinInstances, ClauseToLabelMap);
				else
			...
			if (flag_GetFlagIntValue(Flags, flag_PDER))
				for (Scan=Derivables; !list_Empty(Scan); Scan=list_Cdr(Scan)) {
					fputs("\nDerived: ", stdout);
					clause_PrintLabels(list_Car(Scan),ClauseToLabelMap);
				}
...
static void top_Flotter(int argc, const char* argv[], LIST InputClauses, HASHMAP ClauseToTermLabelList, DFGDESCRIPTION Description, BOOL CommentDescription)
/**************************************************************
  INPUT:  
  RETURNS: Nothing.
  EFFECT:  
 ***************************************************************/
{
	...
	if (CommentDescription) {
		...
		temp = string_Conc("{*",desc_Name(Description) ? desc_Name(Description) : "");
		param_name = string_Conc(temp,"*}");
		string_StringFree(temp);

		temp = string_Conc("{*",desc_Author(Description) ? desc_Author(Description) : "");
		param_author = string_Conc(temp,"*}");
		string_StringFree(temp);

		temp = string_Conc("{*",desc_Description(Description) ? desc_Description(Description) : "");
		param_description = string_Conc(temp,creator); /*the terminal "*}" comes from creator*/
		string_StringFree(temp);

		clause_FPrintCnfDFGProblem(Output, FALSE, param_name, param_author,
				desc_StatusString(Description), param_description, InputClauses,
				NULL, NULL, NULL, ClauseToTermLabelList, FALSE, TRUE);

		string_StringFree(param_name);
		string_StringFree(param_author);
		string_StringFree(param_description);
	} else {
		int  size;
		int  creator_size;
		...
		/* desc_Description(Description) is a string of the form "{*<something>*}"
      we concatenate that (without the last two characters "*}") with <creator> (which puts them back) */

		creator_size     = (int)strlen(creator);
		description_size = (int)strlen(desc_Description(Description));
		size             = description_size - 2 + creator_size + 1; /* add one for the terminating null */
		description      = (char*)memory_Malloc(sizeof(char)*size);
		strncpy(description,desc_Description(Description),description_size-2);
		strcpy(description+description_size-2, creator);
...
static void top_EstablishClAxRelation(LIST ClAxRelation, LIST InputClauses, LIST* Labellist, HASHMAP ClauseToTermLabellist, BOOL DocProof)
/**************************************************************
  INPUT:   A list of relations between clause numbers and formula labels (strings) <ClAxRelation>
           ...
           a hashmap relating clauses to formula labels (strings)
           the doc proof flag
  RETURNS: Nothing.
  EFFECT:  If <DocProof> and the <ClAxRelation> is not empty, then the
           ...
           in the hashmap <ClauseToTermLabellist> and the labels are collected in  <Labellist>
	...
	if (!list_Empty(ClAxRelation)) {
		if (DocProof) {
			for (Scan1=ClAxRelation; !list_Empty(Scan1);Scan1=list_Cdr(Scan1)) {
				...
				hm_InsertListAppend(ClauseToTermLabellist, Clause, list_Cdr(list_Car(Scan1)));
			...
			*Labellist = list_PointerDeleteDuplicates(*Labellist);
			for (Scan1=InputClauses; !list_Empty(Scan1);Scan1=list_Cdr(Scan1))
				if (hm_Retrieve(ClauseToTermLabellist, list_Car(Scan1)) == NULL) {
					misc_StartUserErrorReport();
...
int main(int argc, const char* argv[])
{
	...
	FILE*             InputStream;
	char *            IncludePath;
	HASHMAP           TermLabelToClauselist, ClauseToTermLabellist;
	...
	Axioms = Conjectures = Sorts = Labellist = UserPrecedence = UserSelection = ClAxRelation = list_Nil();

	Sortlabellist = list_Nil();

	cmdlne_SetFlags(InputFlags);   /* Needed for TPTP flag triggering TPTP parser, and for IncludePath */
	IncludePath = flag_GetFlagStringValue(InputFlags,flag_IncludePath);
	...
	if (flag_GetFlagIntValue(InputFlags, flag_STDIN)) {
		...
		if (flag_GetFlagIntValue(InputFlags,flag_TPTP) >= flag_ON)
			InputStream = tptp_OpenFile(top_InputFile,IncludePath,&top_DiscoveredFile);
		else
			InputStream = dfg_OpenFile(top_InputFile,IncludePath,&top_DiscoveredFile);
	}
	...
	if (flag_GetFlagIntValue(InputFlags,flag_TPTP) >= flag_ON) { /* TPTP parsing */
		...
		InputClauses = tptp_TPTPParser(InputStream, IncludePath, InputFlags, InputPrecedence, Description, &Axioms,
		...
		InputClauses = dfg_DFGParser(InputStream, IncludePath, InputFlags, InputPrecedence, Description, &Axioms,
				&Conjectures, &Sorts, &UserPrecedence, &UserSelection,
				&ClAxRelation, &NativeClauseInput);
	}

	msorts_processSortDeclarations(Sorts);


	/*
    Sorts is list with Declarations for new Sorts.
    As it not used in anyway, it will be deleted directly.
	 */
	...
	/* Set nextclause flags if isabelle is set */
	nextclause_setFlags(InputFlags,Flags);

//  TODO sortcheck
//	printf("isabelle: %d\n",flag_GetFlagIntValue(Flags, flag_ISABELLE));
//	printf("isabelle: %d\n",flag_GetFlagIntValue(Flags, flag_DepthMax));
//	if (flag_GetFlagIntValue(InputFlags, flag_ISABELLE) == flag_ISABELLEUNSORTCHECK) {
//		printf("check\n");
//		if (!msorts_SortCheckClauses(InputClauses)) {
//			misc_StartErrorReport();
//			misc_ErrorReport("\n In main: Detected ill-sorted Input Clauses.");
//			misc_FinishErrorReport();
//		}
//	}
	...
	/* printf("\n Ord: %d %s\n", Flags[flag_ORD].intvalue, Flags[flag_ORD].stringvalue); */
	...
	/* Create necessary hashmaps */
	if (flag_GetFlagIntValue(Flags, flag_FPFCR) || flag_GetFlagIntValue(Flags, flag_DOCPROOF) || flag_GetFlagIntValue(Flags, flag_LIGHTDOCPROOF)) {
		TermLabelToClauselist = hm_Create(0, (HM_GET_HASH_FUNCTION)hm_StringHash,
				(HM_KEY_EQUAL_FUNCTION)cnf_LabelEqual, FALSE);
		ClauseToTermLabellist = hm_Create(0, hm_PointerHash, hm_PointerEqual, FALSE);
	}
	else {
	...
	/* Establish clause to term (formula) labels in case of flag flag_FPFCR or flag_DOCPROOF */
	top_EstablishClAxRelation(ClAxRelation,InputClauses, &Labellist, ClauseToTermLabellist,
			(flag_GetFlagIntValue(Flags, flag_FPFCR)|| flag_GetFlagIntValue(Flags, flag_DOCPROOF) || flag_GetFlagIntValue(Flags, flag_LIGHTDOCPROOF)));
	...
	if (list_Empty(InputClauses)) {
		...
		Termcount  = 0;
		/* Old Sort. Could be usefull for new ones.
      for (Scan = Sorts; !list_Empty(Scan); Scan = list_Cdr(Scan), Termcount++) {
      char L[100];
      char* Label;
      sprintf(L, "declaration%zu", Termcount);
      Label = string_StringCopy(L);
      list_Rplaca(list_Car(Scan), Label);
      if (flag_GetFlagIntValue(Flags, flag_DOCPROOF) &&
	  flag_GetFlagIntValue(Flags, flag_PLABELS)) {
...
//		symbol_MSortPrintVars();
//		symbol_MSortPrintFunctions();
	...
	if (top_CalledFlotter(Flags, argv[0]) || flag_GetFlagIntValue(Flags, flag_FLOTTER)) {
		...
		top_Flotter(argc,argv,InputClauses, ClauseToTermLabellist,Description,FromTPTP);
		...
		list_DeleteWithElement(InputClauses, (void (*)(POINTER))clause_Delete);
	...
	LabelClauses = list_Nconc(l, LabelClauses);
			...
			for (Scan=LabelClauses; !list_Empty(Scan); Scan=list_Cdr(Scan)) {
				...
				l = list_Copy(hm_Retrieve(ClauseToTermLabellist, C));
				l = cnf_DeleteDuplicateLabelsFromList(l);
				list_Rplaca(Scan, clause_Copy(C));
				hm_InsertListAppend(ClauseToTermLabellist, list_Car(Scan), l);
			}
		...
		if (flag_GetFlagIntValue(Flags, flag_LOOPS) != 0 && flag_GetFlagIntValue(Flags, flag_TIMELIMIT) != 0)
			Search = top_ProofSearch(Search, ProblemClauses, InputFlags, UserPrecedence, &BoundApplied, NativeClauseInput,ClauseToTermLabellist);
		...
		if (flag_GetFlagIntValue(Flags, flag_TIMELIMIT) != 0 && top_NoAlarm == 0) {
			...
			Scan = ana_GetTransAxClauseList(prfs_GetAnalyze(Search));
			ana_SetTransAxClauseList(prfs_GetAnalyze(Search),list_Nil()); /*taking the clauses from the analyze module (wrt memory)*/
			while(!list_Empty(Scan)) {
				CLAUSE Clause = list_Car(Scan);
				clause_PrecomputeOrderingAndReInit(Clause,Flags,Precedence); /* so that the clause is nice */
				prfs_InsertWorkedOffClause(Search,Clause);
				Scan = list_Pop(Scan);
			}
				...
				if (flag_GetFlagIntValue(Flags, flag_TPTP) == flag_TPTPALL) {
					...
					if (Result == top_COMPLETION)
						clause_FPrintCnfDFGProblem(Output, PrintPotProductive,
								...
								(flag_GetFlagIntValue(Flags, flag_FPFCR) ? ClauseToTermLabellist : NULL),
								TRUE, TRUE);
					else
						clause_FPrintCnfDFGProblem(Output, PrintPotProductive,
...
//			if (flag_GetFlagIntValue(Flags, flag_LIGHTDOCPROOF)) {
//				nextclauseweights_printLabelsList(prfs_EmptyClauses(Search));
//			}
			if (flag_GetFlagIntValue(Flags, flag_DOCPROOF) && Result != top_RESOURCE) {
				if (Result == top_COMPLETION) {
					puts("\n\n The saturated set of worked-off clauses is :");
					clause_ListPrint(prfs_WorkedOffClauses(Search));
					if (ana_GetFeature(prfs_GetAnalyze(Search),ana_PUREPROPOSITIONAL)) {
						printf("\n\n");
						clause_PrintPropModel(prfs_WorkedOffClauses(Search),Flags,Precedence);
					}
					...
					for (Scan = UsedClauses; !list_Empty(Scan); Scan = list_Cdr(Scan))
						if (clause_IsFromInput((CLAUSE) list_Car(Scan))) {
							LIST L;
							L = hm_Retrieve(ClauseToTermLabellist, list_Car(Scan));
							L = list_Copy(L);
							L = cnf_DeleteDuplicateLabelsFromList(L);
		...
		for (Scan = LabelClauses; !list_Empty(Scan); Scan=list_Cdr(Scan))
			hm_RemoveListDelete(ClauseToTermLabellist,list_Car(Scan));
	...
	if (TermLabelToClauselist != (HASHMAP)NULL) {
		hm_DeleteList(TermLabelToClauselist);
		hm_DeleteList(ClauseToTermLabellist);
	}
