...
#define DFG2ASCII__VERSION "0.48"
...
int main(int argc, const char* argv[])
{
  ...
  Clauses  = dfg_DFGParser(Input, IncludePath, Flags, Precedence, Description, &Axioms, &Conjectures,
  ...
  if (!list_Empty(Axioms) || !list_Empty(Conjectures)) {
    ...
    */
    ...
    for (Scan = Clauses;!list_Empty(Scan);Scan=list_Cdr(Scan)) {
      ...
      clause_PrecomputeOrderingAndReInit(Clause, Flags, Precedence);
    }
    puts("\n\n\t\t Axiom Clauses:\n");
    SetExist = FALSE;
    for (Scan = Clauses;!list_Empty(Scan);Scan=list_Cdr(Scan)) {
