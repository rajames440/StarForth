...
struct
  ...
  fun error msg = MLRiscErrorMsg.error("AMD64OmitFramePointer", msg)
  ...
  fun omitframeptr{vfp:CB.cell, idelta:Int32.int option, cfg as Graph.GRAPH graph} = let
    ...
    fun rewrite(instrs, idelta) = let
      ...
      fun whichSize(size, cell) = which(cell)
      fun either(cell) = isSp(cell) orelse isVfp(cell)
      fun eitherSize(size, cell) = either(cell)
	 ...
	 *)
	fun addToDelta i = (case delta
	   of SOME d => SOME(i+d)
	    | NONE => NONE
	  (*esac*))

	fun incOffset i = (case delta
...
*)
	   ...
	   | I.MOVE{mvOp, src, dst as I.Direct d} =>
	       if eitherSize(d) then error "MOVE: assignment to FP/SP"
...
*)
	   ...
	   | I.BINARY{binOp=I.ADDQ, src=I.Immed(k), dst=I.Direct(d)} =>
	     (case whichSize d
	      of SP => changedto(instr, addToDelta(~k))
	       | FP => (NONE, SOME(incOffset(k)))
	       | OTHER => unchanged(instr)
	     (*esac*))
	   | I.BINARY{binOp=I.SUBQ, src=I.Immed(k), dst=I.Direct(d)} =>
	     (case whichSize d
	      of SP => changedto(instr, addToDelta(k))
	       | FP => (NONE, SOME(incOffset(~k)))
	       | OTHER => unchanged(instr)
	   ...
	   | I.BINARY{binOp, dst as I.Direct(d), src} =>
	     if eitherSize(d) then let
		 val (sz,cell) = d
		 val spm = if isSp(cell) then "SP" else ""
		 val fpm = if isVfp(cell) then "FP" else ""
		 val bm =
		     (case binOp
		      of I.ADDQ => "ADDQ"
		       | I.SUBQ => "SUBQ"
		 ...
		 val srcm =
		     (case src
		      of I.Immed x => Int32.toString x
		       | _ => "?")
		 val m = bm^" "^srcm^" "^spm^fpm
	     in
		 error ("binary: assignment "^m)
	     end
...
*)
	   | I.FMOVE{fmvOp:I.fmove_op, src:I.operand, dst:I.operand} =>
	       unchanged(I.FMOVE{fmvOp=fmvOp, src=operand(src), dst=operand(dst)})
(*
  ...
  in
    ...
    case !col
     of CB.PSEUDO => app (fn nid => dfs(nid, idelta)) (#entries graph ())
      | CB.MACHINE x => error ("virtual frame pointer not a pseudo register (machine "^Int.toString x^")")
