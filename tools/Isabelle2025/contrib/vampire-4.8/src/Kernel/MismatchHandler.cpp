...
namespace Kernel {
    ...
    public:
        AcIter(unsigned function, TermList t,
               RobSubstitutionTL const *subs) : _function(function), _todo(), _subs(subs) { _todo->push(t); }
    ...
    Option<MismatchHandler::AbstractionResult> MismatchHandler::tryAbstract(
        RobSubstitutionTL *sub, TermList const &t1, TermList const &t2) const {
        ...
        if (_mode == Uwa::AC1 || _mode == Uwa::AC2) {
            ...
            auto &diff1 = *diff1_;
            auto &diff2 = *diff2_;
            diff1.moveFromIterator(iterSortedDiff(arrayIter(a1), arrayIter(a2)).map([](auto &x) -> TermList {
                return x;
            }));
            diff2.moveFromIterator(iterSortedDiff(arrayIter(a2), arrayIter(a1)).map([](auto &x) -> TermList {
                return x;
            }));
            auto sum = [](auto &diff) {
            ...
            auto diffConstr = [&]() { return UnifConstraint(sum(diff1), sum(diff2)); };

            auto functors = [](auto &diff) { return arrayIter(diff).map([](auto &f) { return f.term()->functor(); }); };
    ...
    namespace UnificationAlgorithms {
        ...
        SubstIterator AbstractingUnification::unifiers(TermList t1, TermList t2, RobSubstitutionTL *sub,
                                                       bool topLevelCheck) {
                ...
                while (toDo->isNonEmpty()) {
                    ...
                    if (sub->sameTermContent(dt1, dt2)) {
                        ...
                        if (absRes->is<MismatchHandler::NeverEqual>()) {
                            ...
                            auto eq = [](UnificationConstraint<TermList, VarBank> &c, TermList const &lhs,
                                         TermList const &rhs) {
