 ...
 */
Signature::Symbol::Symbol(const vstring &nm, unsigned arity, bool interpreted, bool stringConstant,
 ...
 */
Signature::Signature() : _foolConstantsDefined(false), _foolTrue(0), _foolFalse(0),
                         _funs(32),
                         _preds(32),
                         _typeCons(32),
...
unsigned Signature::addInterpretedFunction(Interpretation interpretation, OperatorType *type, const vstring &name) {
    ...
    if (_iSymbols.find(mi, res)) {
        ...
        if (name != functionName(res)) {
            USER_ERROR(
                "Interpreted function '" + functionName(res) + "' has the same interpretation as '" + name +
                "' should have");
        }
        return res;
    }

    vstring symbolKey = name + "_i" + Int::toString(interpretation) + (Theory::isPolymorphic(interpretation)
                                                                           ? type->toString()
                                                                           : "");
...
unsigned Signature::addInterpretedPredicate(Interpretation interpretation, OperatorType *type, const vstring &name) {
    ...
    if (_iSymbols.find(mi, res)) {
        ...
        if (name != predicateName(res)) {
            USER_ERROR(
                "Interpreted predicate '" + predicateName(res) + "' has the same interpretation as '" + name +
                "' should have");
        }
        return res;
    }
