...
namespace FMB {
    ...
    FiniteModelMultiSorted::FiniteModelMultiSorted(DHMap<unsigned, unsigned> sizes) : _sizes(sizes), _isPartial(false) {
    ...
    vstring FiniteModelMultiSorted::toString() {
        ...
        for (unsigned f = 0; f < env.signature->functions(); f++) {
            ...
            if (res > 0) {
                modelStm << "tff(" << append(name, "_definition") << ",axiom," << name << " = " << cname << ")." <<
                        endl;
            } else {
        ...
        fModelLabel:
            for (unsigned i = arity - 1; i + 1 != 0; i--) {
                ...
                if (args[i] == _sizes.get(argS)) {
                    ...
                    if (res > 0) {
                        if (!first) { modelStm << "         & "; } else { modelStm << "           "; }
                        first = false;
                    } else {
                        modelStm << "%         ";
                    }
    ...
    bool FiniteModelMultiSorted::evaluateGroundLiteral(Literal *lit) {
        ...
        for (unsigned i = 0; i < arity; i++) {
            ...
            if (args[i] == 0)
                USER_ERROR("Could not evaluate " + lit->toString() +
