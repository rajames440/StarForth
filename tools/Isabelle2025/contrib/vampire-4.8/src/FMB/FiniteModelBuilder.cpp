...
namespace FMB {
    FiniteModelBuilder::FiniteModelBuilder(Problem &prb, const Options &opt)
        ...
        if (prop.hasInterpretedOperations()
            ...
            if (outputAllowed()) {
                ...
                env.out() << "WARNING: trying to run FMB on interpreted or otherwise provably infinite-domain problem!"
                        << endl;
                env.endOutput();
            }
    ...
    bool FiniteModelBuilder::reset() {
        ...
        for (unsigned f = 0; f < env.signature->functions(); f++) {
            ...
            f_offsets[f] = offsets;
#if VTRACE_FMB
            cout << "offset for " << f << " is " << offsets << " (arity is " << env.signature->functionArity(f) << ") "
                    << endl;
#endif
            ...
            for (unsigned i = 1; i < f_signature.size(); i++) {
                ...
                if (n_add < add) {
                    // additional overflow check - we multiply by positive integers!
                    return false;
                }
                add = n_add;
            }
...
*/
    ...
    void FiniteModelBuilder::init() {
        ...
        // preprocessing should preserve sorts and doing this here means that introduced symbols get sorts
        {
            ...
            for (unsigned s = 0; s < env.signature->typeCons(); s++) {
                if ((env.property->usesSort(s) || env.signature->isNonDefaultCon(s)) && _sortedSignature->
                    vampireToDistinct.find(s)) {
            ...
            while (cit.hasNext()) {
                ...
                for (unsigned i = 0; i < c->length(); i++) {
                    ...
                    if (lit->isEquality()) {
                        ...
                        if (csig_set[var]) { ASS_EQ((*csig)[var], ret); } else {
                            (*csig)[var] = ret;
                            csig_set[var] = true;
                        }
                        for (unsigned j = 0; j < t->arity(); j++) {
                            ...
                            ASS(avar < csig->size());
                            if (!csig_set[var]) { ASS((*csig)[avar] == asrt); } else {
                                (*csig)[avar] = asrt;
                                csig_set[avar] = true;
                            }
                        }
                        ...
                        for (unsigned j = 0; j < lit->arity(); j++) {
                            ...
                            if (csig_set[avar]) { ASS((*csig)[avar] == asrt); } else {
                                (*csig)[avar] = asrt;
                                csig_set[avar] = true;
                            }
                        }
                    ...
                    if (csig_set[var1]) {
                        if (csig_set[var2]) {
                            ...
                            if ((*csig)[var1] != (*csig)[var2]) {
                                ...
                                if ((*csig)[var1] == sort) { (*csig)[var1] = (*csig)[var2]; } else {
                                    (*csig)[var2] = (*csig)[var1];
                                }
                            }
                        } else {
            ...
            for (unsigned var = 0; var < vars; var++) {
                ...
                if (!_xmass) {
                    ...
                    if (!_sortedSignature->monotonicSorts[dsort]) {
                        // don't mark instances of monotonic sorts!
    ...
    unsigned FiniteModelBuilder::estimateFunctionalDefCount() {
        ...
        for (unsigned f = 0; f < env.signature->functions(); f++) {
            ...
            instances *= min(_sortedSignature->sortBounds[returnSrt],
                             _distinctSortSizes[_sortedSignature->parents[returnSrt]]);
            instances *= min(_sortedSignature->sortBounds[returnSrt],
                             _distinctSortSizes[_sortedSignature->parents[returnSrt]]);
        ...
        for (unsigned f = 0; f < env.signature->functions(); f++) {
            ...
            if (arity == 0) {
                ...
                for (unsigned i = (!_xmass || (_sortedSignature->monotonicSorts[dsrt])) ? maxSize : 1; i <= maxSize; i
                     ++) {
                    // just the weakest one, if monotonic
                    static SATLiteralStack satClauseLits;
                    satClauseLits.reset();
                    ...
                    if (_xmass) {
                        ...
                        // use the largest marker for the largest version even if it is smaller than _distinctSortSizes[dsrt]
        ...
        newTotalLabel:
            for (unsigned var = arity - 1; var + 1 != 0; var--) {
                if (grounding[var] == maxVarSize[var]) {
                    ...
                    for (unsigned i = (!_xmass || (_sortedSignature->monotonicSorts[dRetSrt])) ? maxRtSrtSize : 1;
                         i <= maxRtSrtSize; i++) {
                        static SATLiteralStack satClauseLits;
                        satClauseLits.reset();
 ...
 */
    ...
    MainLoopResult FiniteModelBuilder::runImpl() {
        ...
        if (!_xmass) {
            if (!_dsaEnumerator->init(_startModelSize, _distinctSortSizes, _distinct_sort_constraints,
                                      _strict_distinct_sort_constraints)) {
                goto gave_up;
            }
        }
                ...
                if (satResult == SATSolver::SATISFIABLE) {
                    if (_xmass) {
                        ...
                        for (unsigned i = 0; i < _distinctSortSizes.size(); i++) {
                            ...
                            // at the latest "marker_offsets[i]+_distinctSortSizes[i]-1" must have been false (see the assumptions above)
                    ...
                    if (_xmass) {
                        ...
                        if (domsWeight < UINT_MAX) {
                            ...
                            _distinctSortSizes[domToGrow]++; {
                                ...
                                do {
                                    ...
                                    Stack<std::pair<unsigned, unsigned> >::Iterator it2(
                                        _strict_distinct_sort_constraints);
                                    while (it1.hasNext()) {
                                        std::pair<unsigned, unsigned> constr = it1.next();
                                        if (_distinctSortSizes[constr.first] <= _distinctSortSizes[constr.second]) {
                            ...
                            Clause *empty = new(0) Clause(
                                0, NonspecificInference0(UnitInputType::AXIOM, InferenceRule::MODEL_NOT_FOUND));
                        ...
                        for (unsigned i = 0; i < failed.size(); i++) {
                            ...
                            if (var < instancesMarker_offset) {
                                // totality used (-> instances used as well / unless the sort is monotonic)
                                ...
                                // instances used (and we don't know yet about totality)
  ...
  */
    ...
    bool FiniteModelBuilder::HackyDSAE::checkConstriant(DArray<unsigned> &newSortSizes,
                                                        Constraint_Generator_Vals &constraint) {
    ...
    bool FiniteModelBuilder::HackyDSAE::increaseModelSizes(DArray<unsigned> &newSortSizes,
                                                           DArray<unsigned> &sortMaxes) {
        ...
        while (!_constraints_generators.isEmpty()) {
                ...
                if (_keepOldGenerators) {
                    for (unsigned n = 0; n < _old_generators.size(); n++) {
                        if (checkConstriant(newSortSizes, _old_generators[n]->_vals)) {
                            ...
                            Constraint_Generator *gen_p = new Constraint_Generator(
                                newSortSizes.size(), ++_maxWeightSoFar);
                ...
                // test 3 -- (strict)_distinct_sort_constraints
                {
                    ...
                    while (it1.hasNext()) {
                        ...
                        if (newSortSizes[constr.first] < newSortSizes[constr.second]) {
#if VTRACE_DOMAINS
                            cout << "  Ruled out by _distinct_sort_constraints " << constr.first << " >= " << constr.
                                    second << endl;
#endif
                            ...
                            Constraint_Generator *gen_p = new Constraint_Generator(
                                newSortSizes.size(),
                                ++_maxWeightSoFar /*effectively a fallback to FIFO for artificial children*/);
                            Constraint_Generator_Vals &gen = gen_p->_vals;
                            for (unsigned j = 0; j < newSortSizes.size(); j++) {
            ...
            if (_keepOldGenerators) {
                ...
                // keeping old generators degraded performance on average ...
            } else {
    ...
    bool FiniteModelBuilder::SmtBasedDSAE::init(unsigned _startModelSize, DArray<unsigned> &_distinctSortSizes,
                                                ...
                                                Stack<std::pair<unsigned, unsigned> > &
                                                _strict_distinct_sort_constraints) {
 ...
 *  and return the weight of the vector */
    ...
    bool FiniteModelBuilder::SmtBasedDSAE::increaseModelSizes(DArray<unsigned> &newSortSizes,
                                                              DArray<unsigned> &sortMaxes) {
