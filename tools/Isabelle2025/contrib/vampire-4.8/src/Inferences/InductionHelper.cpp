...
namespace Inferences {
    namespace {
        struct SLQueryResultToTermQueryResultFn {
            ...
            TermQueryResult operator()(const SLQueryResult slqr) {
                return TermQueryResult(slqr.unifier->applyToQuery(variable), slqr.literal, slqr.clause,
    ...
    TermQueryResultIterator InductionHelper::getComparisonMatch(
        ...
        Literal *pattern = Literal::create2(less, polarity, termIsLeft ? TermList(t) : var,
                                            termIsLeft ? var : TermList(t));
        return pvi(getMappingIterator(
            _comparisonIndex->getUnifications(pattern, /*complementary=*/ false, /*retrieveSubstitution=*/ true),
    ...
    static bool termAndLiteralSatisfyStrictness(const TermList &tl, Literal *l,
                                                Options::IntegerInductionLiteralStrictness strictness) {
