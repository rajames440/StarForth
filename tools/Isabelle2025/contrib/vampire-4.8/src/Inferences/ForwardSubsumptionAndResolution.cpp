...
namespace Inferences {
    ...
    Clause *ForwardSubsumptionAndResolution::generateSubsumptionResolutionClause(
        Clause *cl, Literal *lit, Clause *baseClause) {
    ...
    bool ForwardSubsumptionAndResolution::perform(Clause *cl, Clause *&replacement, ClauseIterator &premises) {
        ...
        for (unsigned li = 0; li < clen; li++) {
...
#if VHOL
                    env.property->higherOrder() ? _unitIndex->getHOLGeneralizations((*cl)[li], false, false) :
#endif
            ...
            for (unsigned li = 0; li < clen; li++) {
                SLQueryResultIterator rit =
#if VHOL
                        env.property->higherOrder() ? _fwIndex->getHOLGeneralizations((*cl)[li], false, false) :
#endif
                ...
                while (rit.hasNext()) {
                    ...
                    if (MLMatcher::canBeMatched(mcl, cl, cms->_matches, 0) && ColorHelper::compatible(
                            cl->color(), mcl->color())) {
            ...
            if (!_subsumptionResolution) {
                ...
                for (unsigned li = 0; li < clen; li++) {
                    ...
                    SLQueryResultIterator rit =
#if VHOL
                            env.property->higherOrder() ? _unitIndex->getHOLGeneralizations(resLit, true, false) :
#endif
                    ...
                    while (csit.hasNext()) {
                        ...
                        for (unsigned li = 0; li < clen; li++) {
                            Literal *resLit = (*cl)[li];
                            if (checkForSubsumptionResolution(cl, cms, resLit) && ColorHelper::compatible(
                                    cl->color(), cms->_cl->color())) {
                ...
                for (unsigned li = 0; li < clen; li++) {
                    ...
                    SLQueryResultIterator rit =
#if VHOL
                            env.property->higherOrder() ? _fwIndex->getHOLGeneralizations(resLit, true, false) :
#endif
