...
row_init(void)
{
	...
	stack_INIT(row);
#ifndef HW_VERSION
	for (i = 0; i < row->alloc; ++i) LAsigned_mp_init(row->data[i].a);
#endif
	return row;
}
...
#ifndef HW_VERSION
	unsigned i;
	for (i = 0; i < row->alloc; ++i) LAsigned_mp_clear(row->data[i].a);
#endif
	stack_free(row);
}
...
#ifndef HW_VERSION
	for (i = row->size; i < new->alloc; ++i) LAsigned_mp_init(new->data[i].a);
#endif
	row_free(row);
	return new;
}
...
static inline void
simplex_var_update(
	Tsimplex_var i, TLAdelta_mp* D, TLAsigned_mp num, TLAsigned_mp den)
{
	int bound;
...
linear_expr_normalize_mp(Trow row)
{
	...
	/* compute gcd */
	for (i = 1; LAsigned_mp_notone(store.normalize.tmp) && i < ROW_LEN(row);
			 ++i) {
...
static void
row_update_aux(
	Trow* P, const Trow L_i, const TLAsigned_mp a, const TLAsigned_mp b,
	const unsigned offset)
{
	...
	while (it_k < ROW_LEN(*P) || it_i < ROW_LEN(L_i)) {
		if (
			...
			LAsigned_mp_set(ROW_COEF(store.row_update.row, it), ROW_COEF(*P, it_k));
			...
			if (LAsigned_mp_is_zero(store.row_update.a)) {
				...
				row_inc(store.row_update.row);
				ROW_VAR(store.row_update.row, it) = ROW_VAR(*P, it_k);
				LAsigned_mp_set(ROW_COEF(store.row_update.row, it), store.row_update.a);
				++it;
				++it_k;
				++it_i;
			}
	...
	Tsimplex_var k, Tsimplex_var i, TLAsigned_mp a, TLAsigned_mp b)
{
	...
	if (offset == ROW_LEN(L_k)) {
		...
		for (it = 0, it_k = offset; it < ROW_LEN(store.row_update.row);
			...
			ROW_ID(L_k, it_k) = k;
			LAsigned_mp_set(ROW_COEF(L_k, it_k), ROW_COEF(store.row_update.row, it));
			mon_C_add(&stack_get(L_k, it_k));
		}
...
static void
row_update_passive(
	const Tsimplex_var k, const Tsimplex_var i, const TLAsigned_mp a,
		...
		*Prow = row_realloc(*Prow, ROW_LEN(*Prow) + ROW_LEN(L_i));
		...
		while (it_k < ROW_LEN(*Prow) || it_i < ROW_LEN(L_i)) {
			if (
				...
				LAsigned_mp_set(store.row_update.a, ROW_COEF(*Prow, it_k));
				LAsigned_mp_com(store.row_update.a, ROW_COEF(L_i, it_i), a, b);
				if (!LAsigned_mp_is_zero(store.row_update.a)) {
					row_inc(store.row_update.row);
					ROW_VAR(store.row_update.row, it) = ROW_VAR(*Prow, it_k);
	...
	if (simplex_conflict_direct) {
		...
		while (i < ROW_LEN(row)) {
			if (ROW_VAR(row, i) != simplex_conflict_var) {
				bool opposite =
					LAsigned_mp_sign_diff(ROW_COEF(row, i), store.conflict.c);
				/* using xor to negate
...
dispatch_reason_proof(
	Tsimplex_var var, bool upper, Tstack_lit* Pconflict,
	Tstack_simplex_var* Pvar_eq, Tstack_LAsigned_mp* Pconflict_coeffs,
	Tstack_LAsigned_mp* Peq_coeffs,	TLAsigned_mp coeff)
{
	Tlit reason = simplex_var[var].reason[upper];
	/* reason is a literal, or an equality taken care of by LA interface,
     or a bound to branch around non-integral values for integer variables */
	TLAsigned_mp b;
	LAsigned_mp_init(b);
	LAsigned_mp_abs(b, coeff);
	if (reason) {
		stack_push(*Pconflict, reason);

		stack_inc(*Pconflict_coeffs);
		LAsigned_mp_init(stack_top(*Pconflict_coeffs));
		LAsigned_mp_set(stack_top(*Pconflict_coeffs), b);
	} else {
		stack_push(*Pvar_eq, var);

		stack_inc(*Peq_coeffs);
		LAsigned_mp_init(stack_top(*Peq_coeffs));

		LAsigned_mp_set(stack_top(*Peq_coeffs), b);

		if(!upper)
			LAsigned_mp_set(stack_top(*Peq_coeffs), b);
		else
			LAsigned_mp_neg(stack_top(*Peq_coeffs), b);
	}
	LAsigned_mp_clear(b);
}

static inline void
push_num(Tstack_LAsigned_mp* Pcoeffs, TLAsigned_mp coeff)
{
	stack_inc(*Pcoeffs);
	LAsigned_mp_init(stack_top(*Pcoeffs));
	if (!LAsigned_mp_is_zero(coeff)) {
		LAsigned_mp_set(stack_top(*Pcoeffs), coeff);
	}
	else {
		LAsigned_mp_set_si(stack_top(*Pcoeffs), 1L);
	}
}

void
simplex_mp_conflict_proof(
	Tstack_lit* Pconflict, Tstack_simplex_var* Pvar_eq,
	Tstack_LAsigned_mp* Pcoeffs)
{
	Tstack_LAsigned_mp eq_coeffs = NULL;
	assert(simplex_status == UNSAT); /* IMPROVE: UNIFY STATUS EVERYWHERE */
	...
	TLAsigned_mp c;
	LAsigned_mp_init(c);
	if (!LAsigned_mp_is_zero(store.conflict.c)) {
		LAsigned_mp_set(c, store.conflict.c);
		dispatch_reason_proof(
			simplex_conflict_var, simplex_conflict_upper, Pconflict, Pvar_eq,
			Pcoeffs, &eq_coeffs, c);
	}
	...
	if(eq_coeffs) {
		...
		for (unsigned i = 0; i < stack_size(eq_coeffs); ++i) {
			stack_inc(*Pcoeffs);
			LAsigned_mp_init(stack_top(*Pcoeffs));
			LAsigned_mp_set(stack_top(*Pcoeffs), stack_get(eq_coeffs, i));
			LAsigned_mp_clear(stack_get(eq_coeffs, i));
		}
...
backtrack_bound(
	...
	TLAdelta_mp* bound, unsigned char mask)
{
...
backtrack_upper_bound(
	Tsimplex_var var, Tlit reason, TLAdelta_mp* bound, unsigned char mask)
{
...
update(Tsimplex_var var, TLAdelta_mp* v)
{
	...
	if (LAdelta_mp_is_zero(&store.update.D)) return;
	Pmon = COL(var);
	while (Pmon) {
...
static inline bool
pivot_candidate(
	Tsimplex_var i, Tsimplex_var j, TLAsigned_mp ci, TLAsigned_mp cj, bool upp)
{
	if (i == j) return false;
	if (LAsigned_mp_sign_diff(ci, cj)) upp = !upp;
...
static inline void
solve_bound(
	Tsimplex_var i, /*TLAsigned_mp ci,*/
...
simplex_mp_solve(void)
{
	...
	while (1) {
		...
		if (
			...
			IS_BOUND_UPP(i) &&
			LAdelta_mp_less(&simplex_var[i].bound[UPP], &simplex_var[i].assign))
...
assert_equality(void)
{
	...
	for (j = 0; j < ROW_LEN(store.constraint_push.row); ++j) {
		...
		ROW_ID(row_v, j) = v;
		LAsigned_mp_set(ROW_COEF(row_v, j), ROW_COEF(store.constraint_push.row, j));
...
simplex_mp_constraint_push(unsigned n, Tsimplex_var* vars, TLAsigned_mp* coefs)
{
	...
	if (!n) my_error("simplex_mp_constraint_push: inconsistency\n");
...
simplex_mp_simp_const(void)
{
	...
	for (var = 1; var < simplex_var_n; ++var) {
		...
		if (PASSIVE(var) || NONBASIC(var) || !simplex_mp_var_fixed(var)) continue;
		...
		for (i = 0; i < ROW_LEN(row); ++i) {
			...
			if (!simplex_mp_var_fixed(var2)) break; /* ... and it is not fixed */
		}
...
{
	stats_unsigned(
		"simplex_var_mp", "Number of variables for simplex (MP version)", "%9u",
		simplex_var_n);
	stats_unsigned(
		"simplex_var_mp_unbound",
		"Number of unbounded eliminated variables for simplex (MP version)", "%9u",
		simplex_var_n_unbound);
	stats_unsigned(
		"simplex_var_mp_const",
		"Number of constant eliminated variables for simplex (MP version)", "%9u",
		simplex_var_n_const);
	stats_unsigned(
		"simp_MP_piv", "simplex MP pivoting operations", "%9u", simplex_pivot);
	stats_unsigned(
		"simp_MP_piv_h", "simplex MP pivot heuristics", "%9u",
...
LA_invariant(void)
{
	...
	for (i = 1; i < simplex_var_n; i++) {
		...
		if (simplex_var[i].boundmask == 3)
			assert(
				LAdelta_mp_leq(&simplex_var[i].bound[LOW], &simplex_var[i].bound[UPP]));
		...
		if (NONBASIC(i) || !simplex_var[i].mark_unsat) {
			...
			if (IS_BOUND_LOW(i))
				assert(
					LAdelta_mp_leq(&simplex_var[i].bound[LOW], &simplex_var[i].assign));
			if (IS_BOUND_UPP(i))
				assert(
					LAdelta_mp_leq(&simplex_var[i].assign, &simplex_var[i].bound[UPP]));
		}
#ifdef SIMPLEX_COPY
